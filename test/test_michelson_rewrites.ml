...
let oasis_micheline_to_micheline oasis_micheline =
  ...
  let micheline =
    Tezos_micheline.Micheline.map_node
      (fun x -> x)
      (fun prim -> Lltz_michelson.Ast.Prim.of_string prim)
      (If_suffix_rewriter.optimize
         (Tezos_micheline.Micheline.map_node
            (fun x -> x)
            (fun prim -> Lltz_michelson.Ast.Prim.to_string prim)
            micheline))
  in
  micheline
;;
  ...
  let micheline =
    Smartpy_core.Micheline.Sequence
      (Smartpy_core.Michelson.To_micheline.instruction
         { instr = MIseq (List.map ~f:(fun i -> { instr = i }) instr) })
  in
  micheline
;;
...
let test_instructions instructions =
  ...
  let optimised_code =
    Smartpy_core.Michelson_rewriter.run Smartpy_core.Michelson_rewriter.simplify code
  in
  let oasis_micheline_list =
    Smartpy_core.Michelson.To_micheline.instruction optimised_code
  in
  ...
  let second_optimised_code =
    Smartpy_core.Michelson_rewriter.run
      Smartpy_core.Michelson_rewriter.simplify
      optimised_code
  in
  let second_optimised_micheline_list =
    Smartpy_core.Michelson.To_micheline.instruction second_optimised_code
  in
  let second_optimised_oasis_micheline =
    Smartpy_core.Micheline.Sequence second_optimised_micheline_list
  in
  if second_optimised_oasis_micheline <> optimised_oasis_micheline
  then (
    Printf.printf "Further optimisations were possible:\n";
    Test_nodes.print_instructions
      [ oasis_micheline_to_micheline optimised_oasis_micheline ];
    Printf.printf "->\n";
    Test_nodes.print_instructions
      [ oasis_micheline_to_micheline second_optimised_oasis_micheline ])
;;
...
let%expect_test "push_push_same_literal" =
  test [ MIpush (mt_nat, MLiteral.small_int 42); MIpush (mt_nat, MLiteral.small_int 42) ];
  [%expect {|
...
let%expect_test "push_zero_compare_nat_eq" =
  test [ MIpush (mt_nat, MLiteral.small_int 0); MIdig 1; MI2 Compare; MI1 Eq ];
  [%expect
    {|
...
let%expect_test "push_zero_compare_int_ge" =
  test [ MIpush (mt_int, MLiteral.small_int 0); MIdup 2; MI2 Compare; MI1 Ge ];
  [%expect {|
...
let%expect_test "cond_check_last_same_push_if" =
  test
    [ MIif
        ( { instr =
              MIseq
                [ mk_instr (MIdig 2); mk_instr (MIpush (mt_int, MLiteral.small_int 42)) ]
          }
        , { instr =
              MIseq
                [ mk_instr (MIdig 1); mk_instr (MIpush (mt_int, MLiteral.small_int 42)) ]
          } )
    ];
  [%expect
    {|
...
let%expect_test "cond_check_last_push_fail_if" =
  test
    [ MIif
        ...
        , { instr = MIseq [ mk_instr (MI1_fail Failwith) ] } )
    ];
  [%expect
    {|
...
let%expect_test "cond_check_last_fail_push_if" =
  test
    [ MIif
        ...
        , { instr =
              MIseq
                [ mk_instr (MIdig 2); mk_instr (MIpush (mt_nat, MLiteral.small_int 123)) ]
          } )
    ];
  [%expect
    {|
...
let%expect_test "cond_check_last_same_push_if_none" =
  test
    [ MIif_none
        ( { instr =
              MIseq
                [ mk_instr (MIdig 3); mk_instr (MIpush (mt_int, MLiteral.small_int 99)) ]
          }
        , { instr =
              MIseq
                [ mk_instr (MIdig 1); mk_instr (MIpush (mt_int, MLiteral.small_int 99)) ]
          } )
    ];
  [%expect
    {|
...
let%expect_test "cond_check_last_fail_push_if_none" =
  test
    [ MIif_none
        ( { instr = MIseq [ mk_instr (MI1_fail Failwith) ] }
        , { instr =
              MIseq [ mk_instr (MIdup 1); mk_instr (MIpush (mt_unit, MLiteral.unit)) ]
          } )
    ];
  [%expect
    {|
...
let%expect_test "cond_check_last_same_push_if_left" =
  test
    [ MIif_left
        ( { instr =
              MIseq [ mk_instr MIdrop; mk_instr (MIpush (mt_bool, MLiteral.bool true)) ]
          }
        , { instr =
              MIseq
                [ mk_instr (MIdup 1); mk_instr (MIpush (mt_bool, MLiteral.bool true)) ]
          } )
    ];
  [%expect
    {|
...
let%expect_test "cond_check_last_fail_push_if_left" =
  test
    [ MIif_left
        ...
        , { instr =
              MIseq
                [ mk_instr (MIdig 3); mk_instr (MIpush (mt_nat, MLiteral.small_int 7)) ]
          } )
    ];
  [%expect
    {|
...
let%expect_test "cond_check_last_fail_push_if_cons" =
  test
    [ MIif_cons
        ( { instr = MIseq [ mk_instr (MI1_fail Failwith) ] }
        , { instr =
              MIseq [ mk_instr (MIdig 2); mk_instr (MIpush (mt_unit, MLiteral.unit)) ]
          } )
    ];
  [%expect
    {|
...
let%expect_test "if_with_drop_appended_after_the_if" =
  test_instructions
    [ MIif
        ( { instr = MIseq [ mk (MIpush (mt_int, MLiteral.small_int 1)); mk (MI2 Add) ] }
        , { instr = MIseq [ mk (MIpush (mt_int, MLiteral.small_int 2)); mk (MI2 Mul) ] }
        )
    ; MIdrop
    ];
  [%expect
...
let%expect_test "if_left_with_drop_appended" =
  test_instructions
    [ MIif_left
        ( { instr = MIdrop }
        , { instr = MIseq [ mk (MIpush (mt_nat, MLiteral.small_int 42)); mk (MI1 Int) ] }
        )
    ; MIdrop
    ];
  [%expect
...
let%expect_test "if_cons_with_drop_appended" =
  test_instructions
    [ MIif_cons ({ instr = MIseq [ mk (MIdig 2); mk MIswap ] }, { instr = MIdrop })
    ; MIdrop
    ];
  [%expect
    {|
...
let%expect_test "if_with_prefix_drop_on_left_and_failing_right" =
  test_instructions
    [ MIif
        ( { instr = MIseq [ mk MIdrop; mk (MI2 Or) ] }
        , { instr =
              MIseq
...
let%expect_test "if_with_prefix_drop_on_right_and_failing_left" =
  test_instructions
    [ MIif
        ...
        , { instr = MIseq [ mk MIdrop; mk (MI1 Not) ] } )
    ];
  [%expect
    {|
...
let%expect_test "if_none_with_prefix_drop_on_left_and_failing_right" =
  test_instructions
    [ MIif_none
        ( { instr = MIseq [ mk MIdrop; mk (MIdup 2) ] }
        , { instr =
              MIseq
...
let%expect_test "if_left_with_prefix_drop_on_right_and_failing_left" =
  test_instructions
    [ MIif_left
        ...
        , { instr = MIseq [ mk MIdrop; mk (MIdig 3) ] } )
    ];
  [%expect
    {|
...
let%expect_test "if_cons_with_prefix_drop_on_left_and_failing_right" =
  test_instructions
    [ MIif_cons
        ( { instr = MIseq [ mk MIdrop; mk (MIpush (mt_bool, MLiteral.bool true)) ] }
        , { instr =
              MIseq
...
let%expect_test "remove_prefix_drop_on_if_left" =
  test_instructions
    [ MIif_left
        ({ instr = MIdrop }, { instr = MIseq [ mk (MIpush (mt_unit, MLiteral.unit)) ] })
    ];
  [%expect
    {|
...
let%expect_test "dig_dug_several_same_DIG" =
  test_instructions [ MIdig 2; MIdig 2; MIdig 2; MIdig 2 ];
  [%expect {|
...
let%expect_test "dig_dug_several_same_DUG" =
  test_instructions [ MIdug 3; MIdug 3; MIdug 3 ];
  [%expect {|
    { DUG 3 ; DUG 3 ; DUG 3 }
    ->
    { DIG 3 } |}]
...
let%expect_test "dig_dug_interleave" =
  test_instructions [ MIdig 2; MIdig 2; MIdug 2; MIdug 2; MIdig 2 ];
  [%expect {|
...
let%expect_test "dig_dug_with_swap" =
  test_instructions [ MIdig 1; MIdig 1; MIswap; MIdug 1; MIdug 1 ];
  [%expect {|
...
let%expect_test "dig_dug_swap_with_break" =
  test_instructions [ MIdig 1; MIpush (mt_nat, MLiteral.small_int 42); MIswap; MIdug 1 ];
  [%expect
    {|
...
let%expect_test "dig_dug_single_DIG" =
  test_instructions [ MIdig 3; MIpush (mt_string, MLiteral.string "hello") ];
  [%expect
    {|
...
let%expect_test "dig_dug_single_DUG" =
  test_instructions [ MIdug 2; MI1 Not; MIdrop ];
  [%expect {|
    { DUG 2 ; NOT ; DROP }
    ->
    { SWAP ; DROP ; SWAP } |}]
...
let%expect_test "dig_dug_cycle_dig" =
  test_instructions [ MIdig 3; MIdig 3; MIdig 3 ];
  [%expect {|
    { DIG 3 ; DIG 3 ; DIG 3 }
    ->
    { DUG 3 } |}]
...
let%expect_test "dig_dug_cycle_dug" =
  test_instructions [ MIdug 2; MIdug 2 ];
  [%expect {|
    { DUG 2 ; DUG 2 }
    ->
    { DIG 2 } |}]
...
let%expect_test "if_DIG2_DROP_vs_DIG1_DROP_minmax_like_case" =
  test_instructions
    [ MIif
        ( { instr =
              MIseq
                [ mk (MIdig 2); mk MIdrop; mk (MIpush (mt_int, MLiteral.small_int 100)) ]
          }
        , { instr =
              MIseq
                [ mk (MIdig 1)
...
let%expect_test "if_min_max_pattern" =
  test_instructions
    [ MIif
        ( { instr = MIseq [ mk MIdrop; mk (MIdig 3); mk MIdrop ] }
        , { instr = MIseq [ mk (MIdig 1); mk MIdrop; mk (MIdig 3); mk MIdrop ] } )
    ];
  [%expect
    {|
...
let%expect_test "if_with_empty_branches" =
  test_instructions [ MIif ({ instr = MIseq [] }, { instr = MIseq [] }) ];
  [%expect {|
    { IF {} {} }
    ->
    { DROP } |}]
...
let%expect_test "if_cons_double_drop_left_empty_right" =
  test_instructions
    [ MIif_cons ({ instr = MIseq [ mk MIdrop; mk MIdrop ] }, { instr = MIseq [] }) ];
  [%expect {|
    { IF_CONS { DROP ; DROP } {} }
    ->
    { DROP } |}]
...
let%expect_test "if_push_bool_x_then_afterwards_not_on_stack" =
  test_instructions
    [ MIif (mk (MIpush (mt_bool, MLiteral.bool true)), mk MIdrop); MI1 Not ];
  [%expect
    {|
...
let%expect_test "if_drop_appended_afterwards" =
  test_instructions
    [ MIif
        ( { instr = MIseq [ mk (MIpush (mt_string, MLiteral.string "left code")) ] }
        , { instr = MIseq [ mk (MIpush (mt_string, MLiteral.string "right code")) ] } )
    ; MIdrop
    ];
  [%expect
    {|
...
let%expect_test "if_dig_drop_appended_afterwards" =
  test_instructions
    [ MIif
        ( { instr = MIseq [ mk (MIpush (mt_nat, MLiteral.small_int 123)) ] }
        , { instr = MIseq [ mk (MIpush (mt_string, MLiteral.string "string-literal")) ] }
        )
    ; MIdig 2
    ; MIdrop
    ];
...
let%expect_test "if_left_left_branch_fail_right_branch_dig_drop" =
  test_instructions
    [ MIif_left
        ...
        , { instr =
              MIseq
                [ mk (MIdig 3); mk MIdrop; mk (MIpush (mt_int, MLiteral.small_int 111)) ]
          } )
    ];
  [%expect
    {|
...
let%expect_test "if_none_pair_fail_test" =
  test_instructions
    [ MIif_none
        ( { instr = MIseq [ mk (MI2 (Pair (None, None))); mk (MI1_fail Failwith) ] }
        , { instr =
              MIseq
                [ mk (MIdig 3); mk MIdrop; mk (MIpush (mt_nat, MLiteral.small_int 999)) ]
          } )
    ];
  [%expect
    {|
...
let%expect_test "if_drop_fail" =
  test_instructions
    [ MIif
        ( { instr = MIseq [ mk MIdrop; mk (MI1 Neg) ] }
        , { instr =
              MIseq
...
let%expect_test "if_fail_drop" =
  test_instructions
    [ MIif
        ( { instr = MIseq [ mk (MI1_fail Failwith) ] }
        , { instr = MIseq [ mk MIdrop; mk (MIdup 1) ] } )
    ];
  [%expect
    {|
...
let%expect_test "if_both_branches_drop_merge_to_single_drop" =
  test_instructions
    [ MIif ({ instr = MIseq [ mk MIdrop; mk MIdrop ] }, { instr = MIseq [ mk MIdrop ] }) ];
  [%expect
    {|
    { IF { DROP ; DROP } { DROP } }
    ->
...
let%expect_test "if_left_push_int_n_fail_merge" =
  test_instructions
    [ MIif_left
        ( { instr =
              MIseq
                [ mk (MIpush (mt_int, MLiteral.small_int 999)); mk (MI1_fail Failwith) ]
          }
        , { instr =
              MIseq
                [ mk (MIpush (mt_int, MLiteral.small_int 999)); mk (MI1_fail Failwith) ]
...
let%expect_test "if_cons_left_is_dig_drop_fail_right" =
  test_instructions
    [ MIif_cons
        ( { instr =
              MIseq
                [ mk (MIdig 2); mk MIdrop; mk (MIpush (mt_bool, MLiteral.bool false)) ]
          }
        , { instr =
              MIseq
...
let%expect_test "if_left_dig1_drop_many" =
  test_instructions
    [ MIif_left
        ( { instr = MIseq [ mk (MIdig 1); mk MIdrop; mk (MIpush (mt_unit, MLiteral.unit)) ]
          }
        , { instr =
              MIseq
                [ mk MIdrop; mk MIdrop; mk (MIpush (mt_string, MLiteral.string "hello")) ]
          } )
    ];
  [%expect
    {|
...
let%expect_test "if_multiple_drop_on_left_and_right" =
  test_instructions
    [ MIif_none
        ...
        , { instr =
              MIseq
                [ mk (MIdig 1); mk MIdrop; mk (MIpush (mt_nat, MLiteral.small_int 42)) ]
          } )
    ];
  [%expect
    {|
...
let%expect_test "if_left_drop_drop_and_drop_drop" =
  test_instructions
    [ MIif_left
        ( { instr =
              MIseq [ mk MIdrop; mk MIdrop; mk (MIpush (mt_int, MLiteral.small_int 10)) ]
          }
        , { instr =
              MIseq [ mk MIdrop; mk MIdrop; mk (MIpush (mt_int, MLiteral.small_int 20)) ]
          } )
    ];
  [%expect
    {|
...
let%expect_test "if_left_with_lambda_in_branch" =
  ...
  test_instructions
    [ MIif_left
        ( { instr = MIseq [ mk (MIlambda (mt_nat, mt_int, lam_body)); mk (MI2 Exec) ] }
...
let%expect_test "if_left_with_lam_and_fail" =
  ...
  test_instructions
    [ MIif_left
        ( { instr =
              MIseq [ mk (MIlambda (mt_nat, mt_int, lam_body)); mk (MI1_fail Failwith) ]
          }
        , { instr =
              MIseq
                [ mk (MIpush (mt_int, MLiteral.small_int 999)); mk (MI1_fail Failwith) ]
...
let%expect_test "if_drop_on_one_branch_and_nested_if_on_other" =
  test_instructions
    [ MIif
        ( { instr =
              MIseq
                [ mk MIdrop; mk (MIpush (mt_string, MLiteral.string "some leftover")) ]
          }
        , { instr =
              MIseq
                [ mk
...
let%expect_test "if_with_chained_drops_and_fail" =
  test_instructions
    [ MIif
        ( { instr =
              MIseq [ mk MIdrop; mk MIdrop; mk (MIpush (mt_int, MLiteral.small_int 111)) ]
          }
        , { instr =
              MIseq
                [ mk (MIpush (mt_string, MLiteral.string "fail msg"))
...
let%expect_test "if_none_multiple_branches_with_drop_extra" =
  test_instructions
    [ MIif_none
        ( { instr =
              MIseq [ mk MIdrop; mk (MIpush (mt_string, MLiteral.string "some data")) ]
          }
        , { instr =
              MIseq
                [ mk (MIdig 2)
...
let%expect_test "if_cons_push_bool_false_fail_merge" =
  test_instructions
    [ MIif_cons
        ( { instr =
              MIseq [ mk (MIpush (mt_bool, MLiteral.bool false)); mk (MI1_fail Failwith) ]
          }
        , { instr =
              MIseq [ mk (MIpush (mt_bool, MLiteral.bool false)); mk (MI1_fail Failwith) ]
          } )
...
let%expect_test "if_left_drop_dig1_drop_left_vs_simple_right" =
  test_instructions
    [ MIif_left
        ( { instr = MIseq [ mk MIdrop; mk (MIdig 1); mk MIdrop ] }
        , mk (MIpush (mt_nat, MLiteral.small_int 42)) )
    ];
  [%expect
    {|
...
let%expect_test "remove_comments_single" =
  test_instructions [ MIcomment [ "this is a comment" ] ];
  [%expect {|
    {}
    ->
    {} |}]
...
let%expect_test "remove_comments_sequences" =
  test_instructions [ MIcomment [ "comment in seq" ]; MIswap ];
  [%expect {|
    { SWAP }
    ->
    { SWAP } |}]
...
let%expect_test "main_superfluous_swap" =
  test_instructions
    [ MIdup 1; MIdig 1; MIdrop; MIpush (mt_string, MLiteral.string "post-swap") ];
  [%expect
    {|
...
let%expect_test "main_one_to_one_instr_drop" =
  test_instructions [ MI1 Not; MIdrop; MIpush (mt_int, MLiteral.small_int 42) ];
  [%expect {|
...
let%expect_test "main_two_to_one_instr_drop" =
  test_instructions [ MI2 Compare; MIdrop; MIpush (mt_bool, MLiteral.bool true) ];
  [%expect
    {|
...
let%expect_test "main_two_to_one_instr_double_drop" =
  test_instructions
    [ MI2 Compare; MIdrop; MIdrop; MIpush (mt_string, MLiteral.string "hello") ];
  [%expect
    {|
...
let%expect_test "main_merge_consecutive_DIP" =
  test_instructions
    [ MIdip { instr = MIseq [ mk (MIpush (mt_nat, MLiteral.small_int 10)); mk MIdrop ] }
    ; MIdip
        { instr = MIseq [ mk (MIpush (mt_string, MLiteral.string "hello")); mk (MI1 Not) ]
        }
    ; MIdrop
    ];
  [%expect
...
let%expect_test "main_DIP_with_DROP_only" =
  test_instructions [ MIdip { instr = MIdrop }; MIpush (mt_int, MLiteral.small_int 999) ];
  [%expect
    {|
...
let%expect_test "main_DUP1_DIP_with_1to1_instr" =
  test_instructions [ MIdup 1; MIdip { instr = MIseq [ mk (MI1 Not) ] }; MIdrop ];
  [%expect {|
     { DUP ; DIP { NOT } ; DROP }
     ->
     { NOT } |}]
...
let%expect_test "main_loop_push_bool_false" =
  test_instructions
    ...
    ; MIloop { instr = MIseq [ mk (MI2 Mul); mk MIdrop ] }
...
let%expect_test "main_push_literal_then_Some" =
  test_instructions [ MIpush (mt_int, MLiteral.small_int 10); MI1 Some_; MIdrop ];
  [%expect {|
...
let%expect_test "main_two_one_instr_drop_drop" =
  test_instructions
    [ MI2 Compare; MIdrop; MIdrop; MIpush (mt_int, MLiteral.small_int 42) ];
  [%expect
    {|
...
let%expect_test "main_three_one_harmless_instr_drop" =
  test_instructions [ MI3 Slice; MIdrop; MIpush (mt_nat, MLiteral.small_int 987) ];
  [%expect {|
...
let%expect_test "main_remove_dip_drop" =
  test_instructions
    [ MIdip { instr = MIdrop }; MIpush (mt_string, MLiteral.string "after dip+drop") ];
  [%expect
    {|
...
let%expect_test "main_get_and_update_drop" =
  test_instructions
    [ MI3 Get_and_update; MIdrop; MIpush (mt_int, MLiteral.small_int 999) ];
  [%expect
    {|
...
let%expect_test "main_mifield_concat" =
  test_instructions
    [ MIfield [ A; D ]; MIfield [ A ]; MIpush (mt_nat, MLiteral.small_int 99) ];
  [%expect
    {|
...
let%expect_test "main_push_false_loop" =
  test_instructions
    [ MIpush (mt_bool, MLiteral.bool false); MIloop { instr = MI2 Sub }; MIdrop ];
  [%expect {|
...
let%expect_test "main_dup1_dip_dup1" =
  test_instructions
    [ MIdup 1; MIdip { instr = MIdup 1 }; MIpush (mt_nat, MLiteral.small_int 77) ];
  [%expect
    {|
...
let%expect_test "main_dig1_commutative_binop" =
  test_instructions [ MIdig 1; MI2 Add; MIdrop ];
  [%expect {|
    { DIG 1 ; ADD ; DROP }
    ->
    { ADD ; DROP } |}]
...
let%expect_test "main_dig1_compare_eq" =
  test_instructions
    [ MIdig 1; MI2 Compare; MI1 Eq; MIpush (mt_string, MLiteral.string "rest") ];
  [%expect
    {|
...
let%expect_test "main_dig1_compare_neq" =
  test_instructions [ MIdig 1; MI2 Compare; MI1 Neq; MIdrop ];
  [%expect {|
...
let%expect_test "main_eq_not_to_neq" =
  test_instructions [ MI1 Eq; MI1 Not; MIpush (mt_int, MLiteral.small_int 200) ];
  [%expect {|
    { EQ ; NOT ; PUSH int 200 }
    ->
    { NEQ ; PUSH int 200 } |}]
...
let%expect_test "main_neq_not_to_eq" =
  test_instructions [ MI1 Neq; MI1 Not; MIpush (mt_int, MLiteral.small_int 111) ];
  [%expect {|
...
let%expect_test "main_dig1_compare_gt" =
  test_instructions [ MIdig 1; MI2 Compare; MI1 Gt; MIpush (mt_bool, MLiteral.bool true) ];
  [%expect
    {|
...
let%expect_test "main_bubble_up_drop_dig1_double_drop" =
  test_instructions
    [ MIdig 1; MIdrop; MIdrop; MIpush (mt_string, MLiteral.string "done") ];
  [%expect
    {|
...
let%expect_test "main_bubble_up_swap_push_mono" =
  test_instructions [ MIpush (mt_nat, MLiteral.small_int 777); MIdig 1; MI1 Not; MIdrop ];
  [%expect
    {|
...
let%expect_test "main_multiple_dig_drops" =
  test_instructions
    [ MIdig 3; MIdig 2; MIdrop; MIpush (mt_string, MLiteral.string "rest") ];
  [%expect
    {|
...
let%expect_test "main_dup1_dig_drop" =
  test_instructions [ MIdup 1; MIdig 3; MIdrop; MIdrop ];
  [%expect {|
    { DUP ; DIG 3 ; DROP ; DROP }
    ->
    { DIG 2 ; DROP } |}]
...
let%expect_test "main_dup_k_dig_n_drop_variants" =
  test_instructions [ MIdup 2; MIdig 3; MIdrop; MIpush (mt_unit, MLiteral.unit) ];
  [%expect
    {|
...
let%expect_test "main_dup_k_dig_n_eq" =
  test_instructions [ MIdup 3; MIdig 3; MIpush (mt_nat, MLiteral.small_int 999) ];
  [%expect
    {|
...
let%expect_test "main_dug_n1_mono_dig_n2" =
  test_instructions
    [ MIdug 3; MI1 Not; MIdig 3; MIpush (mt_string, MLiteral.string "done") ];
  [%expect
    {|
...
let%expect_test "main_dig_n_dig1_mono_dig1" =
  test_instructions [ MIdig 2; MIdig 1; MI1 Neg; MIdig 1; MIdrop ];
  [%expect
    {|
...
let%expect_test "main_dug_midig_drop_mixed" =
  test_instructions
    [ MIdug 3; MIdig 1; MIdrop; MIpush (mt_string, MLiteral.string "rest code") ];
  [%expect
    {|
...
let%expect_test "main_2_1_binop_dig_drop" =
  test_instructions
    [ MI2 Concat2; MIdig 2; MIdrop; MIpush (mt_string, MLiteral.string "final") ];
  [%expect
    {|
...
let%expect_test "main_1_2_instr_dig_drop" =
  test_instructions
    [ MIunpair [ true; true ]; MIdig 3; MIdrop; MIpush (mt_bool, MLiteral.bool true) ];
  [%expect
    {|
...
let%expect_test "main_1_2_instr_dig1_drop_dig_n_drop" =
  test_instructions [ MIunpair [ true; true ]; MIdig 1; MIdrop; MIdig 2; MIdrop; MIdrop ];
  [%expect
    {|
...
let%expect_test "main_1_2_instr_drop_drop" =
  test_instructions
    [ MIpairn 2; MIdig 1; MIdrop; MIdrop; MIpush (mt_string, MLiteral.string "rest") ];
  [%expect
    {|
...
let%expect_test "main_1_2_instr_drop_drop_second" =
  test_instructions
    [ MIunpair [ true; true ]; MIdrop; MIdrop; MIpush (mt_int, MLiteral.small_int 42) ];
  [%expect
    {|
...
let%expect_test "main_1_2_instr_dig_drop_again" =
  test_instructions [ MIunpair [ true; true ]; MIdig 3; MIdrop; MIdrop ];
  [%expect {|
...
let%expect_test "main_mipairn_unpair_alltrue" =
  test_instructions [ MIpairn 3; MIunpair [ true; true; true ]; MIdrop ];
  [%expect {|
...
let%expect_test "main_read_ticket_drop" =
  test_instructions
    [ MI1 Read_ticket; MIdrop; MIpush (mt_string, MLiteral.string "done") ];
  [%expect
    {|
...
let%expect_test "main_dup1_mifield_d_dig1_mifield_a" =
  test_instructions [ MIdup 1; MIfield [ D ]; MIdig 1; MIfield [ A ]; MIdrop ];
  [%expect {|
...
let%expect_test "main_dup1_mifield_a_dig1_mifield_d" =
  test_instructions
    [ MIdup 1; MIfield [ A ]; MIdig 1; MIfield [ D ]; MIpush (mt_unit, MLiteral.unit) ];
  [%expect
    {|
...
let%expect_test "main_ternary_instr_midig_drop" =
  test_instructions
    [ MI3 Update; MIdig 2; MIdrop; MIpush (mt_string, MLiteral.string "rest") ];
  [%expect
    {|
...
let%expect_test "main_ternary_3_2_midig_drop" =
  test_instructions [ MI2 Split_ticket; MIdig 2; MIdrop; MIdrop ];
  [%expect {|
...
let%expect_test "main_dup1_then_fail_redundant" =
  test_instructions [ MIdup 1; MI1_fail Never; MIpush (mt_nat, MLiteral.small_int 100) ];
  [%expect {|
...
let%expect_test "main_dup1_then_failwith" =
  test_instructions [ MIdup 1; MI1_fail Failwith; MIdrop ];
  [%expect {|
    { DUP ; FAILWITH ; DROP }
    ->
    { FAILWITH } |}]
...
let%expect_test "main_comment_drop_reorder" =
  test_instructions
    [ MIcomment [ "some cmt" ]; MIdrop; MIpush (mt_string, MLiteral.string "rest") ];
  [%expect
    {|
...
let%expect_test "main_comment_dig1_reorder" =
  test_instructions [ MIcomment [ "cc" ]; MIdig 1; MIpush (mt_bool, MLiteral.bool false) ];
  [%expect {|
...
let%expect_test "main_comment_dig_n_drop_reorder" =
  test_instructions [ MIcomment [ "c" ]; MIdig 3; MIdrop; MIdrop ];
  [%expect {|
    { DIG 3 ; DROP ; DROP }
    ->
    { DIG 3 ; DROP 2 } |}]
...
let%expect_test "main_mono_dig_drop" =
  test_instructions [ MI1 Not; MIdig 2; MIdrop; MIpush (mt_nat, MLiteral.small_int 999) ];
  [%expect
    {|
...
let%expect_test "main_dug_n_drop" =
  test_instructions [ MIdug 2; MIdrop; MIpush (mt_string, MLiteral.string "post") ];
  [%expect
    {|
...
let%expect_test "main_dup1_dug_n_drop" =
  test_instructions
    [ MIdup 1; MIdug 2; MIdrop; MIpush (mt_string, MLiteral.string "rest") ];
  [%expect
    {|
...
let%expect_test "main_dup1_dip_dig1" =
  test_instructions [ MIdup 1; MIdip { instr = MIdig 1 }; MIdrop ];
  [%expect {|
    { DUP ; DIP { DIG 1 } ; DROP }
    ->
    { SWAP } |}]
...
let%expect_test "main_dup1_push_binop_dig1_drop" =
  test_instructions
    [ MIdup 1; MIpush (mt_int, MLiteral.small_int 444); MI2 Add; MIdig 1; MIdrop; MIdrop ];
  [%expect
    {|
...
let%expect_test "main_complex_dig_dug_interplay" =
  test_instructions [ MIdig 2; MIdig 1; MIdup 1; MIdug 1; MIdrop ];
  [%expect {|
...
let%expect_test "main_constant_folding_int_gt" =
  test_instructions [ MIpush (mt_int, MLiteral.small_int 1); MI1 Gt; MIdrop ];
  [%expect {|
    { PUSH int 1 ; GT ; DROP }
    ->
    {} |}]
...
let%expect_test "main_two_dig_with_drops_n1_gt_n2" =
  test_instructions [ MIdig 3; MIdrop; MIdig 1; MIdrop; MIpush (mt_unit, MLiteral.unit) ];
  [%expect
    {|
...
let%expect_test "main_dig1_dup1_dug2" =
  test_instructions [ MIdig 1; MIdup 1; MIdug 2; MIdrop ];
  [%expect {|
...
let%expect_test "main_dup2_dig1_drop" =
  test_instructions
    [ MIdup 2; MIdig 1; MIdrop; MIpush (mt_string, MLiteral.string "rest") ];
  [%expect
    {|
...
let%expect_test "main_field_concatenation_second_test" =
  test_instructions
    [ MIfield [ A; D ]; MIfield [ D; A ]; MIpush (mt_bool, MLiteral.bool true) ];
  [%expect
    {|
...
let%expect_test "main_nat_neg" =
  test_instructions [ MIpush (mt_nat, MLiteral.small_int 5); MI1 Neg; MIdrop ];
  [%expect {|
    { PUSH nat 5 ; NEG ; DROP }
    ->
    {} |}]
...
let%expect_test "main_nat_int_neg" =
  test_instructions [ MIpush (mt_nat, MLiteral.small_int 12); MI1 Int; MI1 Neg; MIdrop ];
  [%expect {|
...
let%expect_test "main_sub_rewriting_extended" =
  test_instructions [ MIpush (mt_int, MLiteral.small_int 10); MIdig 1; MI2 Sub; MIdrop ];
  [%expect
    {|
...
let%expect_test "main_neg_dig1_add_to_sub" =
  test_instructions
    [ MI1 Neg; MIdig 1; MI2 Add; MIpush (mt_string, MLiteral.string "rest") ];
  [%expect
    {|
...
let%expect_test "main_neg_dig1_sub_to_add" =
  test_instructions [ MI1 Neg; MIdig 1; MI2 Sub; MIdrop ];
  [%expect {|
    { NEG ; DIG 1 ; SUB ; DROP }
    ->
    { ADD ; DROP } |}]
...
let%expect_test "main_push_instr_dig1_exec" =
  ...
  test_instructions
    [ MIpush (mt_lambda mt_unit mt_int, MLiteral.instr lambda_body)
    ; MIdig 1
    ; MI2 Exec
    ; MIdrop
    ];
...
let%expect_test "main_dig_n_dup1_dug_n_plus_1" =
  test_instructions [ MIdig 2; MIdup 1; MIdug 3; MIdrop ];
  [%expect {|
...
let%expect_test "main_dup_n_swap_drop" =
  test_instructions
    [ MIdup 3; MIswap; MIdrop; MIpush (mt_string, MLiteral.string "rest") ];
  [%expect
    {|
...
let%expect_test "main_dup1_dup2" =
  test_instructions [ MIdup 1; MIdup 2; MIdrop ];
  [%expect {|
    { DUP ; DUP 2 ; DROP }
    ->
    { DUP } |}]
...
let%expect_test "main_self_none_address" =
  test_instructions [ MI0 (Self None); MI1 Address; MIdrop ];
  [%expect {|
    { SELF ; ADDRESS ; DROP }
    ->
    {} |}]
...
let%expect_test "main_1_2_instr_drop_drop" =
  test_instructions
    [ MIpairn 2; MIdrop; MIdrop; MIpush (mt_string, MLiteral.string "continuation") ];
  [%expect
    {|
...
let%expect_test "main_1_2_instr_dig1_drop_drop" =
  test_instructions
    [ MIpairn 2; MIdig 1; MIdrop; MIdrop; MIpush (mt_string, MLiteral.string "done") ];
  [%expect
    {|
...
let%expect_test "final_two_one_instr_double_drop_again" =
  test_instructions
    [ MI2 (Pair (None, None)); MIdrop; MIdrop; MIpush (mt_int, MLiteral.small_int 10) ];
  [%expect
    {|
...
let%expect_test "final_three_one_instr_drop" =
  test_instructions
    [ MI3 Slice; MIdrop; MIpush (mt_string, MLiteral.string "some next instr") ];
  [%expect
    {|
...
let%expect_test "final_remove_dip_drop" =
  test_instructions [ MIdip { instr = MIdrop }; MIpush (mt_nat, MLiteral.small_int 123) ];
  [%expect
    {|
...
let%expect_test "final_remove_dip_empty_seq" =
  test_instructions [ MIdip { instr = MIseq [] }; MIdrop ];
  [%expect {|
    { DIP {} ; DROP }
    ->
    { DROP } |}]
...
let%expect_test "final_push_literal_some" =
  test_instructions [ MIpush (mt_nat, MLiteral.small_int 99); MI1 Some_; MIdrop ];
  [%expect {|
...
let%expect_test "final_push_literal_right" =
  test_instructions
    [ MIpush (mt_bool, MLiteral.bool true); MI1 (Right (None, None, mt_nat)); MIdrop ];
  [%expect {|
...
let%expect_test "final_push_false_loop" =
  test_instructions
    [ MIpush (mt_bool, MLiteral.bool false); MIloop { instr = MIswap }; MIdrop ];
  [%expect {|
...
let%expect_test "final_dig1_compare_eq" =
  test_instructions [ MIdig 1; MI2 Compare; MI1 Eq; MIdrop ];
  [%expect {|
...
let%expect_test "final_push_dig_n_greater1" =
  test_instructions
    [ MIpush (mt_string, MLiteral.string "some-literal"); MIdig 3; MIdrop; MIdrop ];
  [%expect
    {|
...
let%expect_test "final_dup1_dig_n_drop" =
  test_instructions [ MIdup 1; MIdig 2; MIdrop; MIpush (mt_unit, MLiteral.unit) ];
  [%expect
    {|
...
let%expect_test "final_one_to_two_instr_dig_n_drop" =
  test_instructions [ MIunpair [ true; true ]; MIdig 2; MIdrop; MIdrop ];
  [%expect {|
...
let%expect_test "final_read_ticket_drop" =
  test_instructions [ MI1 Read_ticket; MIdrop; MIpush (mt_bool, MLiteral.bool false) ];
  [%expect
    {|
...
let%expect_test "final_ternary_midig_drop" =
  test_instructions
    [ MI3 Update; MIdig 2; MIdrop; MIpush (mt_string, MLiteral.string "end") ];
  [%expect
    {|
...
let%expect_test "final_dup1_then_never_fail" =
  test_instructions [ MIdup 1; MI1_fail Never; MIpush (mt_int, MLiteral.small_int 101) ];
  [%expect {|
...
let%expect_test "final_dup1_then_failwith" =
  test_instructions
    [ MIdup 1; MI1_fail Failwith; MIpush (mt_string, MLiteral.string "unreachable") ];
  [%expect
    {|
...
let%expect_test "final_comment_drop_reorder" =
  test_instructions
    [ MIcomment [ "a comment" ]; MIdrop; MIpush (mt_int, MLiteral.small_int 555) ];
  [%expect {|
    { DROP ; PUSH int 555 }
    ->
    { DROP ; PUSH int 555 } |}]
...
let%expect_test "final_comment_dig1" =
  test_instructions
    [ MIcomment [ "some text" ]; MIdig 1; MIpush (mt_nat, MLiteral.small_int 123) ];
  [%expect {|
    { DIG 1 ; PUSH nat 123 }
    ->
    { SWAP ; PUSH nat 123 } |}]
...
let%expect_test "final_comment_dig_drop" =
  test_instructions
    [ MIcomment [ "cc" ]; MIdig 2; MIdrop; MIpush (mt_string, MLiteral.string "rest") ];
  [%expect
    {|
...
let%expect_test "final_ternary_arbitrary" =
  test_instructions
    [ MI2 Split_ticket; MIdig 3; MIdrop; MIdrop; MIpush (mt_int, MLiteral.small_int 777) ];
  [%expect
    {|
...
let%expect_test "final_constant_folding_neg_nat" =
  test_instructions [ MIpush (mt_nat, MLiteral.small_int 10); MI1 Neg; MIdrop ];
  [%expect {|
...
let%expect_test "final_1_2_instr_dig1_double_drop" =
  test_instructions
    [ MIunpair [ true; true ]; MIdig 1; MIdrop; MIdrop; MIpush (mt_unit, MLiteral.unit) ];
  [%expect
    {|
...
let%expect_test "final_lambda_exec_inlining" =
  test_instructions
    [ MIlambda (mt_nat, mt_string, { instr = MI1 Not }); MIdig 1; MI2 Exec; MIdrop ];
  [%expect
    {|
...
let%expect_test "final_dup_n_swap_drop" =
  test_instructions
    [ MIdup 2; MIswap; MIdrop; MIpush (mt_string, MLiteral.string "afterwards") ];
  [%expect
    {|
...
let%expect_test "final_dup1_dup2_againa" =
  test_instructions [ MIdup 1; MIdup 2; MIpush (mt_nat, MLiteral.small_int 444) ];
  [%expect
    {|
...
let%expect_test "lltz_specific_pre_dig_n_dropn_m_when_n_less_than_m" =
  test_instructions [ MIdig 1; MIdropn 3; MIpush (mt_string, MLiteral.string "after") ];
  [%expect
    {|
...
let%expect_test "lltz_specific_pre_dug_n_dropn_m_when_n_less_than_m" =
  test_instructions [ MIdug 2; MIdropn 4; MIpush (mt_nat, MLiteral.small_int 123) ];
  [%expect
    {|
...
let%expect_test "lltz_specific_pre_no_rewrite_when_dig_n_is_not_less_than_m" =
  test_instructions [ MIdig 3; MIdropn 2; MIdrop ];
  [%expect {|
    { DIG 3 ; DROP 2 ; DROP }
    ->
    { DIG 3 ; DROP 3 } |}]
...
let%expect_test "lltz_specific_pre_no_rewrite_when_dug_n_is_not_less_than_m" =
  test_instructions
    [ MIdug 5; MIdropn 5; MIpush (mt_string, MLiteral.string "no rewrite") ];
  [%expect
    {|
...
let%expect_test "lltz_specific_push_option_some" =
  test_instructions
    [ MIpush (mt_option mt_nat, MLiteral.some (MLiteral.small_int 42)); MIdrop ];
  [%expect {|
...
let%expect_test "lltz_specific_push_option_none" =
  test_instructions
    [ MIpush (mt_option mt_string, MLiteral.none); MIpush (mt_bool, MLiteral.bool true) ];
  [%expect
    {|
...
let%expect_test "lltz_specific_list_single_element" =
  test_instructions
    [ MIpush (mt_list mt_nat, MLiteral.list [ MLiteral.small_int 999 ]); MIdig 1 ];
  [%expect
    {|
...
let%expect_test "lltz_specific_list_single_element_more_complex" =
  test_instructions
    [ MIpush (mt_list mt_string, MLiteral.list [ MLiteral.string "only-element" ])
    ; MIdrop
    ];
  [%expect {|
...
let%expect_test "lltz_specific_lambda_int_int_constant_hash" =
  test_instructions
    [ MIlambda
        (mt_int, mt_int, { instr = MIConstant { literal = String "some-hash-here" } })
    ; MIdrop
    ];
  [%expect
    {|
...
let%expect_test "lltz_specific_push_or_left" =
  test_instructions
    [ MIpush (mt_or mt_string mt_nat, MLiteral.left (MLiteral.string "left part"))
    ; MIdig 1
    ];
  [%expect
    {|
...
let%expect_test "lltz_specific_push_or_right" =
  test_instructions
    [ MIpush (mt_or mt_bool mt_int, MLiteral.right (MLiteral.small_int 1234)); MIdrop ];
  [%expect {|
...
let%expect_test "lltz_specific_dug_dig_swap_dropn_more" =
  test_instructions
    [ MIswap; MIdrop; MIdug 1; MIdropn 1; MIpush (mt_nat, MLiteral.small_int 101) ];
  [%expect
    {|
...
let%expect_test "lltz_specific_dug_n_m_dropn_n_eq" =
  test_instructions
    [ MIdig 1; MIdropn 1; MIdug 2; MIdropn 2; MIpush (mt_string, MLiteral.string "done") ];
  [%expect
    {|
...
let%expect_test "lltz_specific_midig2_drop" =
  test_instructions [ MIdig 2; MIdrop; MIpush (mt_bool, MLiteral.bool true) ];
  [%expect
    {|
...
let%expect_test "lltz_specific_midig3_drop" =
  test_instructions
    [ MIdig 3; MIdrop; MIpush (mt_string, MLiteral.string "continuation") ];
  [%expect
    {|
...
let%expect_test "lltz_specific_mipairn_unpair_alltrue" =
  test_instructions
    [ MIpairn 3; MIunpair [ true; true; true ]; MIpush (mt_nat, MLiteral.small_int 777) ];
  [%expect {|
...
let%expect_test "lltz_specific_mdropn_1" =
  test_instructions [ MIdropn 1; MIpush (mt_string, MLiteral.string "some code") ];
  [%expect
    {|
...
let%expect_test "lltz_specific_no_rewrite_scenario" =
  test_instructions [ MIpairn 2; MIunpair [ true; true; true ]; MIdrop ];
  [%expect {|
...
let%expect_test "lltz_specific_no_rewrite_edge_case" =
  test_instructions [ MIdig 2; MIdropn 2; MIpush (mt_unit, MLiteral.unit) ];
  [%expect
    {|
...
let rec mk_dig k n = if n = 0 then [] else MIdig k :: mk_dig k (n - 1)
let rec mk_dug k n = if n = 0 then [] else MIdug k :: mk_dug k (n - 1)

let%expect_test "digdug_cycles_dig_k_times" =
  test_instructions (mk_dig 3 3 @ [ MIpush (mt_string, MLiteral.string "rest") ]);
  [%expect
    {|
...
let%expect_test "digdug_cycles_dig1_or_dug1" =
  test_instructions
    [ MIdig 1; MIdig 1; MIdug 1; MIdug 1; MIpush (mt_nat, MLiteral.small_int 42) ];
  [%expect
    {|
...
let%expect_test "digdug_cycles_incomplete_sequence" =
  test_instructions
    [ MIdig 2; MIdig 2; MIpush (mt_string, MLiteral.string "break sequence"); MIdig 2 ];
  [%expect
    {|
...
let%expect_test "digdug_cycles_more_than_needed" =
  test_instructions (mk_dig 3 5 @ [ MIdrop ]);
  [%expect
    {|
...
let%expect_test "digdug_cycles_interspersed_break" =
  test_instructions
    [ MIdig 3; MIdig 3; MIdrop; MIdig 3; MIpush (mt_string, MLiteral.string "end") ];
  [%expect
    {|
...
let%expect_test "digdug_cycles_dug_interspersed" =
  test_instructions
    [ MIdug 2; MIdug 2; MIswap; MIdug 2; MIpush (mt_bool, MLiteral.bool true) ];
  [%expect
    {|
...
let%expect_test "unpair_empty" =
  test_instructions [ MIunpair []; MIpush (mt_nat, MLiteral.small_int 42) ];
  [%expect {|
...
let%expect_test "unpair_single_true" =
  test_instructions [ MIunpair [ true; true ]; MIdrop ];
  [%expect {|
    { UNPAIR ; DROP }
    ->
    { CDR } |}]
...
let%expect_test "unpair_single_false" =
  test_instructions [ MIunpair [ true ]; MIpush (mt_string, MLiteral.string "rest") ];
  [%expect {|
...
let%expect_test "unpair_true_true_drop" =
  test_instructions [ MIunpair [ true; true ]; MIdrop; MIdig 1 ];
  [%expect {|
    { UNPAIR ; DROP ; DIG 1 }
    ->
    { CDR ; SWAP } |}]
...
let%expect_test "unpair_true_false_drop" =
  test_instructions
    [ MIunpair [ true; true ]; MIdrop; MIpush (mt_bool, MLiteral.bool true) ];
  [%expect
    {|
...
let%expect_test "unpair_complex_fields_first_is_true" =
  test_instructions
    [ MIunpair [ true; true; true ]; MIdrop; MIpush (mt_unit, MLiteral.unit) ];
  [%expect
    {|
...
let%expect_test "unpair_complex_fields_first_is_false" =
  test_instructions
    [ MIunpair [ true; true; true ]; MIpush (mt_string, MLiteral.string "rest code") ];
  [%expect
    {|
...
let%expect_test "unpair_no_rewrite_nonmatching_pattern" =
  test_instructions [ MIunpair [ true; true ]; MI2 Add; MIpush (mt_unit, MLiteral.unit) ];
  [%expect {|
...
let%expect_test "composite_test_4_or_and_pair_interactions" =
  test_instructions
    [ MIpush (mt_or mt_string mt_int, MLiteral.right (MLiteral.small_int 10))
    ; MIdig 1
    ; MIdrop
    ; MIpush
...
let%expect_test "composite_test_6_big_map_update_folding" =
  test_instructions
    [ MIpush (mt_map mt_string mt_bool, MLiteral.mk_map [])
...
let%expect_test "composite_test_9_lambda_inlining_plus_arithmetic" =
  test_instructions
    [ MIlambda
        ...
        , { instr =
              MIseq
                [ mk (MIpush (mt_int, MLiteral.small_int 2)); mk (MI2 Mul); mk (MI1 Neg) ]
          } )
    ; MIdig 1
    ; MI2 Exec
    ; MIpush (mt_bool, MLiteral.bool false)
...
let%expect_test "composite_test_10_lltz_specific_clashes" =
  test_instructions
    [ MIpush (mt_list mt_string, MLiteral.list [ MLiteral.string "one-element" ])
    ; MIdrop
    ; MIdig 2
    ; MIdrop
    ; MIdig 2
