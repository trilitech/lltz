...
let print_map_str_ty map =
  Map.to_alist map
  |> List.map ~f:(fun (k, v) ->
    Printf.sprintf "%s : %s" k (Sexplib.Sexp.to_string_hum (Lltz_ir.Type.sexp_of_t v)))
  |> String.concat ~sep:", "
;;
...
let%expect_test "fv lambda with outside var overshadowing" =
  ...
  let lam_expr =
    lambda
      (var "y", nat_ty)
      ~body:(mul (variable (var "y") nat_ty) (variable (var "x") nat_ty))
  in
  let let_expr =
    let_in (var "x") ~rhs:lam_expr ~in_:(variable (var "x") (function_ty nat_ty nat_ty))
  in
...
let%expect_test "fv app self applied lambda overshadowing" =
  ...
  let let_expr =
    let_in
      (var "x")
      ~rhs:(app lam_expr (variable (var "y") nat_ty))
