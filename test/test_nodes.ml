...
let%expect_test "discarded index_address is preserved by optimiser" =
  (* INDEX_ADDRESS mutates the global address registry; dropping an unused
     result must not eliminate the instruction. *)
  let e =
    let_in (var "idx") ~rhs:(index_address (address_const "tz1ABC123")) ~in_:unit
  in
  test_expr e;
  [%expect
    {|
    { PUSH address "tz1ABC123" ; INDEX_ADDRESS ; DROP ; UNIT }
