...
module T = struct
  ...
  let map t ~f =
    match t with
    | Var v -> Var v
    | Let_in { let_var; rhs; in_ } -> Let_in { let_var; rhs = f rhs; in_ = f in_ }
    | Lambda { lam_var; return; body } -> Lambda { lam_var; return; body = f body }
    ...
    | If_bool { condition; if_true; if_false } ->
      If_bool { condition = f condition; if_true = f if_true; if_false = f if_false }
    | If_none { subject; if_none; if_some = x, if_some } ->
      If_none { subject = f subject; if_none = f if_none; if_some = x, f if_some }
    | If_cons { subject; if_empty; if_nonempty = x, xs, if_nonempty } ->
      If_cons
        { subject = f subject; if_empty = f if_empty; if_nonempty = x, xs, f if_nonempty }
    | If_left { subject; if_left = x, if_left; if_right = y, if_right } ->
      If_left { subject = f subject; if_left = x, f if_left; if_right = y, f if_right }
    | While { invariant; body } -> While { invariant = f invariant; body = f body }
    ...
    | Fold_left { collection; init; fold = xs, fold } ->
      Fold_left { collection = f collection; init = f init; fold = xs, f fold }
