...
let mk_type ~range (desc : LLTZ.T.desc) : LLTZ.T.t = { desc; range }
...
module Default = struct
  (* Constants *)
  let unit ~range = create ~range (LLTZ.E.Const Unit) (mk_type ~range LLTZ.T.Unit)
  let bool ~range b = create ~range (LLTZ.E.Const (Bool b)) (mk_type ~range LLTZ.T.Bool)
  ...
  let key ~range s = create ~range (LLTZ.E.Const (Key s)) (mk_type ~range LLTZ.T.Keys)
  ...
  let sapling_transaction_ty ~range memo =
    { LLTZ.T.desc = Sapling_transaction { memo }; range }
  ;;
  ...
  (* Expressions *)
  let variable ~range var var_ty = create ~range (LLTZ.E.Variable var) var_ty
  ...
  let deref ~range var var_ty = create ~range (LLTZ.E.Deref var) var_ty
  ...
  let if_none ~range subject ~none ~some =
    create
      ~range
      (LLTZ.E.If_none { subject; if_none = none; if_some = some })
  ...
  let tuple ~range row =
    create ~range (LLTZ.E.Tuple row) (mk_type ~range (LLTZ.T.Tuple (get_type_row row)))
  ;;
  ...
  let inj ~range context expr =
    create ~range (LLTZ.E.Inj (context, expr)) (mk_type ~range (get_inj_type context))
  ;;
  ...
  let total_voting_power ~range =
    create
      ~range
      (LLTZ.E.Prim (LLTZ.P.Total_voting_power, []))
    ...
    create
      ...
      (mk_type
         ...
         (match value.LLTZ.E.type_.LLTZ.T.desc with
          ...
          | LLTZ.T.Bls12_381_g1 | LLTZ.T.Bls12_381_g2 | LLTZ.T.Bls12_381_fr ->
            value.LLTZ.E.type_.LLTZ.T.desc
          | _ ->
            raise_s [%message "Expected int,nat or BLS12-381 field/group element type"]))
  ;;
  ...
  let eq ~range lhs rhs =
    create
      ~range
      (LLTZ.E.Prim (LLTZ.P.Eq, [ compare ~range lhs rhs ]))
  ...
  let neq ~range lhs rhs =
    create
      ~range
      (LLTZ.E.Prim (LLTZ.P.Neq, [ compare ~range lhs rhs ]))
      (mk_type ~range LLTZ.T.Bool)
  ;;
  ...
  let le ~range lhs rhs =
    create
      ~range
      (LLTZ.E.Prim (LLTZ.P.Le, [ compare ~range lhs rhs ]))
      (mk_type ~range LLTZ.T.Bool)
  ;;
  ...
  let lt ~range lhs rhs =
    create
      ~range
      (LLTZ.E.Prim (LLTZ.P.Lt, [ compare ~range lhs rhs ]))
      (mk_type ~range LLTZ.T.Bool)
  ;;
  ...
  let ge ~range lhs rhs =
    create
      ~range
      (LLTZ.E.Prim (LLTZ.P.Ge, [ compare ~range lhs rhs ]))
      (mk_type ~range LLTZ.T.Bool)
  ;;
  ...
  let gt ~range lhs rhs =
    create
      ~range
      (LLTZ.E.Prim (LLTZ.P.Gt, [ compare ~range lhs rhs ]))
      (mk_type ~range LLTZ.T.Bool)
  ;;
  ...
  let hash_key ~range key =
    create
      ~range
      (LLTZ.E.Prim (LLTZ.P.Hash_key, [ key ]))
  ...
  let getn ~range n _value =
    create ~range (LLTZ.E.Prim (LLTZ.P.Get_n n, [ _value ])) (assert false)
  ;;

  (*TODO*)

  let cast ~range ty value = create ~range (LLTZ.E.Prim (LLTZ.P.Cast ty, [ value ])) ty
  ...
  let lsr_ ~range lhs rhs =
    create ~range (LLTZ.E.Prim (LLTZ.P.Lsr, [ lhs; rhs ])) lhs.LLTZ.E.type_
  ;;

  let lsl_ ~range lhs rhs =
    create ~range (LLTZ.E.Prim (LLTZ.P.Lsl, [ lhs; rhs ])) lhs.LLTZ.E.type_
  ;;

  let xor ~range lhs rhs =
    create ~range (LLTZ.E.Prim (LLTZ.P.Xor, [ lhs; rhs ])) lhs.LLTZ.E.type_
  ;;
  ...
  let and_ ~range lhs rhs =
    create ~range (LLTZ.E.Prim (LLTZ.P.And, [ lhs; rhs ])) rhs.LLTZ.E.type_
  ;;

  let or_ ~range lhs rhs =
    create ~range (LLTZ.E.Prim (LLTZ.P.Or, [ lhs; rhs ])) lhs.LLTZ.E.type_
  ;;
  ...
  let sapling_verify_update ~range transaction state =
    create
      ...
      (let open LLTZ.T in
       ...
       | _ ->
         raise_s
           [%message "Expected matching sapling_transaction and sapling_state types"])
  ;;
         ...
         (LLTZ.T.Option
            (mk_type
               ...
               (LLTZ.T.Tuple
                  (LLTZ.R.Node
                     [ LLTZ.R.Leaf
                         (None, mk_type ~range (LLTZ.T.Ticket ticket.LLTZ.E.type_))
                     ; LLTZ.R.Leaf
                         (None, mk_type ~range (LLTZ.T.Ticket ticket.LLTZ.E.type_))
                     ])))))
  ;;
  ...
  let convert_list (exprs : LLTZ.E.t list) : LLTZ.E.t LLTZ.R.t =
    let converted_row_leaves = List.map ~f:(fun expr -> LLTZ.R.Leaf (None, expr)) exprs in
...
module With_dummy = struct
  ...
  let unit = Default.unit ~range:v
  let bool b = Default.bool ~range:v b
  let nat n = Default.nat ~range:v n
  let int n = Default.int ~range:v n
  let mutez n = Default.mutez ~range:v n
  let string s = Default.string ~range:v s
  let key s = Default.key ~range:v s
  let key_hash s = Default.key_hash ~range:v s
  let bytes s = Default.bytes ~range:v s
  let chain_id s = Default.chain_id ~range:v s
  let address_const s = Default.address_const ~range:v s
  let timestamp s = Default.timestamp ~range:v s
  let bls12_381_g1 s = Default.bls12_381_g1 ~range:v s
  let bls12_381_g2 s = Default.bls12_381_g2 ~range:v s
  let bls12_381_fr s = Default.bls12_381_fr ~range:v s
  let signature s = Default.signature ~range:v s
  let var = Default.var
  ...
  let for_ index ~init ~cond ~update ~body =
    Default.for_ ~range:v index ~init ~cond ~update ~body
  ;;
  ...
  let fold_right collection ~init ~fold =
    Default.fold_right ~range:v collection ~init ~fold
  ;;

  let let_tuple_in components ~rhs ~in_ =
    Default.let_tuple_in ~range:v components ~rhs ~in_
  ;;

  let tuple row = Default.tuple ~range:v row
  let proj tuple ~path = Default.proj ~range:v tuple ~path

  let update_tuple tuple ~component ~update =
    Default.update_tuple ~range:v tuple ~component ~update
  ;;

  let inj context expr = Default.inj ~range:v context expr
  let match_ subject ~cases = Default.match_ ~range:v subject ~cases

  let raw_michelson michelson args return_ty =
    Default.raw_michelson ~range:v michelson args return_ty
  ;;

  let create_contract ~storage ~code ~delegate ~initial_balance ~initial_storage =
    Default.create_contract
      ~range:v
      ~storage
      ~code
      ~delegate
  ...
  let ticket_deprecated content amount = Default.ticket_deprecated ~range:v content amount
