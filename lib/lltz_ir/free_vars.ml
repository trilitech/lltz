...
let free_vars_with_types (expr : LLTZ.E.t) : LLTZ.T.t String.Map.t =
  ...
  let rec loop (expr : LLTZ.E.t) (bound_vars : string list) : LLTZ.T.t String.Map.t =
    ...
    | Variable (Var name) ->
      if List.mem bound_vars name ~equal:String.equal then
        empty
      else
        String.Map.singleton name expr.type_

    | Let_in { let_var = Var var; rhs; in_ } ->
      merge (remove (loop in_ (var :: bound_vars)) [var]) (loop rhs bound_vars)

    | Lambda { lam_var = Var var, var_type; body } ->
      loop body (var :: bound_vars)

    | Lambda_rec { mu_var = Var mu, mu_type; lambda = { lam_var = Var var, lam_var_type; body } } ->
      loop body (mu :: var :: bound_vars)

    | App { abs; arg } ->
      merge (loop abs bound_vars) (loop arg bound_vars)
    ...
    | Let_tuple_in { components; rhs; in_ } ->
      ...
      merge (remove (loop in_ (component_names @ bound_vars)) component_names) (loop rhs bound_vars)

    | Tuple row ->
      compile_row_free_vars_with_types row bound_vars

    | Proj (tuple, _) ->
      loop tuple bound_vars

    | Update { tuple; component; update } ->
      merge (loop tuple bound_vars) (loop update bound_vars)

    | Inj (_, expr) ->
      loop expr bound_vars
    ...
    | Create_contract { storage; code = {lam_var = Var param_var, param_ty ;body =code_body}; delegate; initial_balance; initial_storage } ->
  ...
  and compile_row_cases_free_vars_with_types cases bound_vars =
    match cases with
    | LLTZ.R.Node nodes ->
      List.map nodes ~f:(fun case -> compile_row_cases_free_vars_with_types case bound_vars)
      |> merge_all
