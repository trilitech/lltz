...
let tuple ?(annots = []) types =
  ...
  let rec loop ?(annots = []) types =
    ...
    | type_ :: types, annot1::tl_annots -> T.pair ~annot1 type_ (loop types ~annots:tl_annots)
...
let ors ?(annots = []) types =
  ...
  let rec loop ?(annots = []) types =
    ...
    | type_ :: types, annot1::tl_annots -> T.or_ ~annot1 type_ (loop types ~annots:tl_annots)
