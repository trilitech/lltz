...
let rec traverse_row (row : 'a LLTZ.R.t) (acc : String.Set.t) (f : 'a -> String.Set.t -> String.Set.t) : String.Set.t =
  match row with
  | LLTZ.R.Node rows ->
      List.fold_left ~f:(fun acc row -> traverse_row row acc f) ~init:acc rows
  | LLTZ.R.Leaf (_label_opt, a) ->
      f a acc
...
let collect_all_vars (expr : LLTZ.E.t) : String.Set.t =
  let rec aux (expr : LLTZ.E.t) acc =
    ...
    | Lambda_rec { mu_var = Var mu, _mu_type; lambda = {lam_var = Var var, _lam_var_type; body} } ->
        let acc = aux body acc in
    ...
    | If_none { subject; if_none; if_some = {lam_var = Var var, _var_type; body = some} } ->
        let acc = aux subject acc in
        let acc = aux if_none acc in
        let acc = aux some acc in
        Set.add acc var

    | If_cons { subject; if_empty; if_nonempty = { lam_var1 = Var hd, _var1_ty; lam_var2 = Var tl, _var2_ty; body = nonempty } } ->
        let acc = aux subject acc in
        let acc = aux if_empty acc in
        let acc = aux nonempty acc in
        Set.add (Set.add acc tl) hd

    | If_left { subject; if_left = {lam_var = Var left, _left_ty; body = l}; if_right = {lam_var = Var right, _right_ty; body = r} } ->
        let acc = aux subject acc in
    ...
    | Fold_left { collection; init = init_body; fold = {lam_var = Var var, _var_ty; body = fold_body} } ->
        let acc = aux collection acc in
        let acc = aux init_body acc in
        let acc = aux fold_body acc in
        Set.add acc var

    | Fold_right { collection; init = init_body; fold = {lam_var = Var var, _var_ty; body = fold_body} } ->
        let acc = aux collection acc in
        let acc = aux init_body acc in
        let acc = aux fold_body acc in
        Set.add acc var
    ...
    | Match (subject, cases) ->
        let acc = aux subject acc in
        let acc = traverse_row cases acc (fun lambda acc ->
...
let rec collect_used_vars (expr : LLTZ.E.t) : String.Set.t =
  ...
  | App { abs; arg } ->
      Set.union (collect_used_vars abs) (collect_used_vars arg)
  ...
  | While { cond; body } ->
      Set.union (collect_used_vars cond) (collect_used_vars body)
  ...
  | For { index = Mut_var _; init; cond; update; body } ->
      ...
      Set.union (collect_used_vars init)
        (Set.union (collect_used_vars cond)
  ...
  | Fold_left { collection; init; fold } ->
      Set.union (collect_used_vars collection)
  ...
  | Tuple row ->
      let rec collect_tuple (row : LLTZ.E.t LLTZ.R.t) =
        ...
        | LLTZ.R.Node es ->
            List.fold_left ~f:(fun acc r -> Set.union acc (collect_tuple r)) ~init:String.Set.empty es
      in
      collect_tuple row

  | Proj (tuple, _) ->
      collect_used_vars tuple
  ...
  | Create_contract { storage = _; code; delegate; initial_balance; initial_storage } ->
      Set.union (collect_used_vars delegate)
        (Set.union (collect_used_vars initial_balance)
...
let compute_last_vars (expr : LLTZ.E.t) : LLTZ.E.t =
  ...
  let rec aux (expr : LLTZ.E.t) (live_set: String.Set.t) : LLTZ.E.t =
    let process_subexpr (expr : LLTZ.E.t) live_set =
      ...
      (expr', live_set')
    in
    let mk_annots ?(remove_never_used_vars = String.Set.empty) last_used_vars = { expr.annotations with last_used_vars; remove_never_used_vars } in
    let process_args args live_set =
      let (args_rev', live_set) =
          List.fold_left
    ...
    let adjust_never_used_var (body:LLTZ.E.t) x =
      if not (Set.mem used_vars x) then
        { body with annotations = { body.annotations with remove_never_used_vars = Set.add body.annotations.remove_never_used_vars x } }
      else
        body
    in
    let process_lambda1 lambda live_set =
      ...
      (lambda', live_set')
    in
    let process_lambda2 lambda2 live_set =
    ...
    let merge_branch_last_used (e1 : LLTZ.E.t) (e2: LLTZ.E.t) =
      let to_add1 = Set.diff e2.annotations.last_used_vars e1.annotations.last_used_vars in
      let to_add2 = Set.diff e1.annotations.last_used_vars e2.annotations.last_used_vars in
      { e1 with annotations = { e1.annotations with remove_never_used_vars = Set.union e1.annotations.remove_never_used_vars to_add1 } },
      { e2 with annotations = { e2.annotations with remove_never_used_vars = Set.union e2.annotations.remove_never_used_vars to_add2 } }
    in

    match expr.desc with
    | Variable (Var v) ->
        let is_last_use = (Set.mem live_set v) in
        let last_used_vars = if is_last_use then String.Set.singleton v else String.Set.empty in
    ...
    | Lambda { lam_var = Var x, lam_var_type; body } ->
        ...
        { expr with desc = Lambda { lam_var = (Var x, lam_var_type); body = body' }; annotations = mk_annots last_used_vars }

    | Lambda_rec { mu_var = Var mu, mu_type; lambda } ->
        let lam_var_name = match lambda.lam_var with Var x, _ -> x | _ -> assert false in
        let live_set = Set.add (Set.add live_set mu) lam_var_name in
        let (body', live_set) = process_subexpr lambda.body live_set in
        let last_used_vars = Set.add (add_lambda1 lambda String.Set.empty) mu in
        let lambda' = { lambda with body = body' } in
        { expr with desc = Lambda_rec { mu_var = (Var mu, mu_type); lambda = lambda' }; annotations = mk_annots last_used_vars }

    | App { abs; arg } ->
        (* Process 'arg' first, then 'abs' *)
        let (abs', live_set) = process_subexpr abs live_set in
        let (arg', live_set) = process_subexpr arg live_set in
        let last_used_vars = add_expr abs' (add_expr arg' String.Set.empty) in
        { expr with desc = App { abs = abs'; arg = arg' }; annotations = mk_annots last_used_vars }

    | Const _ ->
        { expr with annotations = mk_annots String.Set.empty }
    ...
    | Prim (primitive, args) ->
        ...
        let last_used_vars =
          List.fold_left
            ~f:(fun acc arg -> add_expr arg acc)
            ~init:String.Set.empty args'
        in
    ...
    | If_bool { condition; if_true; if_false } ->
        ...
        let (condition', live_set) = process_subexpr condition live_set in
        let last_used_vars = add_expr condition' (add_expr if_true' (add_expr if_false' String.Set.empty)) in

        let (if_false', if_true') = merge_branch_last_used if_false' if_true' in
        { expr with desc = If_bool { condition = condition'; if_true = if_true'; if_false = if_false' }; annotations = mk_annots last_used_vars }
    ...
    | If_none { subject; if_none; if_some } ->
        ...
        let last_used_vars = add_expr subject' (add_expr if_none' (add_lambda1 if_some' String.Set.empty)) in

        let (if_none', if_some_body) = merge_branch_last_used if_none' if_some'.body in
        { expr with desc = If_none { subject = subject'; if_none = if_none'; if_some = {if_some' with body = if_some_body} }; annotations = mk_annots last_used_vars }
    ...
    | If_cons { subject; if_empty; if_nonempty } ->
      ...
      let (subject', live_set) = process_subexpr subject live_set in
      let last_used_vars = add_expr subject' (add_expr if_empty' (add_lambda2 if_nonempty' String.Set.empty)) in

      let (if_empty', if_nonempty_body) = merge_branch_last_used if_empty' if_nonempty'.body in
      { expr with desc = If_cons { subject = subject'; if_empty = if_empty'; if_nonempty = {if_nonempty' with body = if_nonempty_body} }; annotations = mk_annots last_used_vars }
    ...
    | If_left { subject; if_left; if_right } ->
      ...
      let live_set = Set.inter live_set_left live_set_right in
      let (subject', live_set) = process_subexpr subject live_set in
      let last_used_vars = add_expr subject' (add_lambda1 if_left' (add_lambda1 if_right' String.Set.empty)) in

      let (if_left_body, if_right_body) = merge_branch_last_used if_left'.body if_right'.body in
      { expr with desc = If_left { subject = subject'; if_left = {if_left' with body = if_left_body}; if_right = {if_right' with body = if_right_body} }; annotations = mk_annots last_used_vars }
    ...
    | For { index = Mut_var x; init; cond; update; body } ->
      ...
      let (cond', _) = process_subexpr cond String.Set.empty in
      { expr with desc = For { index = Mut_var x; init = init''; cond = cond'; update = update'; body = body' }; annotations = mk_annots last_used_vars }
    ...
    | For_each { collection; body } ->
      ...
      let (body'', _) = process_lambda1 body String.Set.empty in
      { expr with desc = For_each { collection = collection'; body = body'' }; annotations = mk_annots last_used_vars }
    ...
    | Map { collection; map } ->
      ...
      { expr with desc = Map { collection = collection'; map = map'' }; annotations = mk_annots last_used_vars }

    | Fold_left { collection; init; fold } ->
      let (fold', live_set) = process_lambda1 fold live_set in
      let (init', live_set) = process_subexpr init live_set in
      let (collection', live_set) = process_subexpr collection live_set in
      let last_used_vars = add_expr collection' (add_expr init' (add_lambda1 fold' String.Set.empty)) in

      let (fold'', _) = process_lambda1 fold String.Set.empty in
      { expr with desc = Fold_left { collection = collection'; init = init'; fold = fold'' }; annotations = mk_annots last_used_vars }
    ...
    | Fold_right { collection; init; fold } ->
      ...
      let (fold'', _) = process_lambda1 fold String.Set.empty in
      { expr with desc = Fold_right { collection = collection'; init = init'; fold = fold'' }; annotations = mk_annots last_used_vars }

    | Let_tuple_in { components; rhs; in_ } ->
      let live_set = List.fold_left ~f:(fun acc (Var x) -> Set.add acc x) ~init:live_set components in
      let (in_', live_set) = process_subexpr in_ live_set in
      let (rhs', live_set) = process_subexpr rhs live_set in
    ...
    | Tuple row -> 
      let rec aux_tuple (row : LLTZ.E.t LLTZ.R.t) (live_set : String.Set.t) : LLTZ.E.t LLTZ.R.t * String.Set.t =
        match row with
        | LLTZ.R.Node rows ->
            let (rows', live_set) = List.fold_left ~f:(fun (rows_acc, live_set) row -> 
              let (row', live_set) = aux_tuple row live_set in
              (row' :: rows_acc, live_set)) ~init:([], live_set) rows
            in
    ...
    | Proj (tuple, path) ->
      let (tuple', live_set) = process_subexpr tuple live_set in
    ...
    | Update { tuple; component; update } -> 
      ...
      { expr with desc = Update { tuple = tuple'; component; update = update' }; annotations = mk_annots last_used_vars }

    | Inj (context, expr_inj) -> (*TODO*) assert false (*{ expr with annotations = mk_annots live_set }*)
    ...
    | Match (subject, cases) -> (*TODO*) assert false (*{ expr with annotations = mk_annots live_set }*)
    ...
    | Raw_michelson { michelson; args } ->
      ...
      let last_used_vars =
        List.fold_left
          ...
          ~init:String.Set.empty args'
      in
      { expr with desc = Raw_michelson { michelson; args = args' }; annotations = mk_annots last_used_vars }
    ...
    | Create_contract { storage; code; delegate; initial_balance; initial_storage } ->
      ...
      let (initial_storage', live_set) = process_subexpr initial_storage live_set in
      let last_used_vars = add_expr delegate' (add_expr initial_balance' (add_expr initial_storage' (add_lambda1 code' String.Set.empty))) in
      { expr with desc = Create_contract { storage; code = code'; delegate = delegate'; initial_balance = initial_balance'; initial_storage = initial_storage' }; annotations = mk_annots last_used_vars }
    ...
    | Global_constant { hash; args } ->
      ...
      let last_used_vars =
        List.fold_left
          ...
          ~init:String.Set.empty args'
      in
      { expr with desc = Global_constant { hash; args = args' }; annotations = mk_annots last_used_vars }
