 ...
 fun expr ->
  seq
    [ (match expr.desc with
       ...
       | Lambda { lam_var = Var var, lam_var_type; body } -> compile_lambda expr
       | Lambda_rec
       ...
       | If_cons
           { subject
           ; if_empty
           ; if_nonempty = { lam_var1 = Var hd; lam_var2 = Var tl; body = nonempty }
           } -> compile_if_cons subject if_empty (hd, tl, nonempty)
       | If_left
           { subject
           ; if_left = { lam_var = Var left; body = l }
           ; if_right = { lam_var = Var right; body = r }
       ...
       | Fold_left
           { collection
           ; init = init_body
           ; fold = { lam_var = Var var; body = fold_body }
...
and compile_variable (name : string) =
  trace ~flag:(String.append "var " name) (Slot.dup (`Ident name))
...
and compile_while_left init_val var body_lambda res_ty =
  seq
    ...
    ; loop_left (seq [ Slot.let_ (`Ident var) ~in_:(compile body_lambda) ])
    ]
...
and compile_lambda expr =
  ...
  | Lambda { lam_var = Var var, lam_var_type; body } ->
    ...
    let environment =
      LLTZ.Free_vars.free_vars_with_types expr |> Map.map ~f:convert_type |> Map.to_alist
    in
    seq
...
and compile_for_each collection var body =
  ...
  trace
    ...
    (seq
       [ coll_instr
       ; iter (seq [ Slot.let_ (`Ident var) ~in_:(seq [ compile body; drop 1 ]) ])
       ; unit
       ])
  ...
  | LLTZ.R.Context.Node (left_val, mid, right_val) ->
    ...
    trace
      ...
      (seq
         ...
         then []
         else List.map (List.rev (List.tl_exn right_instrs_types)) ~f:(fun ty -> right ty)
         ))
