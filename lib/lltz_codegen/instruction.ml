...
let dig_and_keep n stack =
  ...
  let stack =
    ...
    | _ ->
      raise_s
        [%message
          "Instruction.dig_and_keep: invalid stack" (stack : SlotStack.t) (n : int)]
  in
  Config.ok stack
  @@
  match n with
...
let prim ?(message = "") m n instr stack =
  let stack =
    ...
    then
      raise_s
        [%message
          "Instruction.prim: invalid stack"
            (stack : SlotStack.t)
            (m : int)
            (n : int)
            (message : string)];
...
module Slot = struct
  let def (slot : [< Slot.definable ]) ~in_ stack =
    ...
    | _ ->
      raise_s
        [%message
          "Instruction.Slot.def: slot not assignable"
            (stack : SlotStack.t)
            (slot : [< Slot.definable ])]
  ;;
  ...
  let let_ slot ?(unused_set = String.Set.empty) ~in_ =
    ...
    seq [ def slot ~in_:(seq [ collect_immediately; in_ ]); collect slot ]
  ;;
  ...
  let lookup slot ~in_ stack =
    ...
    | None ->
      raise_s
        [%message
          "Instruction.Slot.lookup: slot not found"
            (stack : SlotStack.t)
            (slot : [< Slot.definable ])]
  ;;

  let dup_or_dig slot condition =
    lookup slot ~in_:(fun idx -> if condition then dup idx else dig_and_keep idx)
  ;;
  ...
  let set (slot : [< Slot.definable ]) stack =
    ...
    | None ->
      ...
      (match stack with
       | top :: rest ->
         if Slot.is_assignable top ~to_:slot
         then Config.ok ((slot :> Slot.t) :: rest) []
         else
           raise_s
             [%message
  ...
  let mock_value stack = Config.ok (`Value :: stack) []
end
...
let lambda ~environment ~lam_var ~return_type return stack =
  ...
  let { Config.stack = _; instructions } =
    ...
    seq
      ...
      lambda_stack
  in
  let parameter_type = Type.tuple (List.map environment ~f:snd @ [ snd lam_var ]) in
...
let lambda_rec ~environment ~lam_var ~mu ~return_type ~partial_apps return stack =
  ...
  let { Config.stack = _; instructions } =
    ...
    seq
      ...
      ; Slot.def_all
          defined_slots
          ~in_:(seq [ dig n; make_top_value partial_apps; dug n; return ])
      ; Slot.collect_all defined_slots
      ]
      lambda_stack
  in
...
let create_contract ~storage ~parameter ~code stack =
  ...
  | `Value :: `Value :: `Value :: stack ->
    Config.ok
      (`Value :: `Value :: stack)
      [ I.create_contract parameter storage (code (`Value :: stack)) ]
  | _ ->
    raise_s [%message "Instruction.create_contract: invalid stack" (stack : SlotStack.t)]
;;

let is_nat =
  prim 1 1 I.is_nat (* https://tezos.gitlab.io/michelson-reference/#instr-ISNAT *)
;;
...
let apply =
  prim
    ...
    2
    1
    I.apply (* https://tezos.gitlab.io/michelson-reference/#instr-APPLY *)
;;
...
let exec =
  prim
    ...
    2
    1
    I.exec (* https://tezos.gitlab.io/michelson-reference/#instr-EXEC *)
;;

let unpair =
  prim 1 2 I.unpair (* https://tezos.gitlab.io/michelson-reference/#instr-UNPAIR *)
;;
...
let update =
  prim 3 1 I.update (* https://tezos.gitlab.io/michelson-reference/#instr-UPDATE *)
;;
...
let add =
  prim
    ...
    2
    1
    I.add (* https://tezos.gitlab.io/michelson-reference/#instr-ADD *)
;;
...
let sub =
  prim
    ...
    2
    1
    I.sub (* https://tezos.gitlab.io/michelson-reference/#instr-SUB *)
;;
...
let mul =
  prim
    ...
    2
    1
    I.mul (* https://tezos.gitlab.io/michelson-reference/#instr-MUL *)
;;

let pair = prim ~message:"pair" 2 1 (I.pair ())
...
let get =
  prim
    ...
    2
    1
    I.get (* https://tezos.gitlab.io/michelson-reference/#instr-GET *)
;;

let unpack type_ = prim 1 1 (I.unpack type_)

(* https://tezos.gitlab.io/michelson-reference/#instr-UNPACK *)
...
let ediv =
  prim
    ...
    2
    1
    I.ediv (* https://tezos.gitlab.io/michelson-reference/#instr-EDIV *)
;;
...
let and_ =
  prim
    ...
    2
    1
    I.and_ (* https://tezos.gitlab.io/michelson-reference/#instr-AND *)
;;
...
let or_ =
  prim
    ...
    2
    1
    I.or_ (* https://tezos.gitlab.io/michelson-reference/#instr-OR *)
;;
...
let compare =
  prim
    ...
    2
    1
    I.compare (* https://tezos.gitlab.io/michelson-reference/#instr-COMPARE *)
;;
...
let nil type_ = prim 0 1 (I.nil type_)
...
let cons =
  prim
    ...
    2
    1
    I.cons (* https://tezos.gitlab.io/michelson-reference/#instr-CONS *)
;;
...
let rec micheline_fails
  ...
  | Seq (_, xs) ->
    (match List.last xs with
     ...
     | None -> false)
  | Prim (_, I If, [ l; r ], _)
  | Prim (_, I If_none, [ l; r ], _)
  | Prim (_, I If_left, [ l; r ], _)
  | Prim (_, I If_cons, [ l; r ], _) -> micheline_fails l && micheline_fails r
  | Prim (_, I Map, [ x ], _)
  | Prim (_, I Iter, [ x ], _)
  | Prim (_, I Loop, [ x ], _)
  | Prim (_, I Dip, [ x ], _)
...
let raw_michelson michelson args stack =
  ...
  then
    raise_s
      [%message
        "Instruction.raw_michelson: invalid stack" (stack : SlotStack.t) (n : int)]
  else (
    let top_elements = List.take stack n in
    if List.for_all top_elements ~f:(function
         | `Value -> true
         | _ -> false)
    then (
      let new_stack = `Value :: List.drop stack n in
      if micheline_fails michelson
      then Config.raise [ michelson ]
      else (
...
let global_constant hash args stack = raw_michelson (I.global_constant hash) args stack
  ...
  seq
    [ (fun stack ->
        print_s
          [%message
            (stack : SlotStack.t)
              (String.concat [ Int.to_string trace_point; "=before "; flag ])]
          stack)
    ; t
    ; (fun stack ->
        print_s
          [%message
            (stack : SlotStack.t)
              (String.concat [ Int.to_string trace_point; "=after "; flag ])]
          stack)
    ]
;;
