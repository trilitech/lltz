...
let mk_pred (force_nil, force_z, no_force) x =
  match x with
  | Prim (_, name, _, _) ->
...
(* 3) The eq check used by last_is eq pred.
   ...
   - else false
*)
let mk_eq (force_nil, force_z, no_force) m1 m2 =
  match m1, m2 with
  (* force_nil, no sub-args, same name => eq_type => true *)
  | Prim (_, l, [], _), Prim (_, r, [], _) when List.mem l force_nil && String.equal l r
    -> true
  ...
  | Prim (_, l, [ Int (_, n) ], _), Prim (_, r, [ Int (_, m) ], _)
    when List.mem l force_z && String.equal l r && Z.equal n m -> true
  | _ -> false
;;

(** The “opt_cond” pass that merges last instructions from IF branches. **)
let rec opt_cond_one_pass ?pre_type (node : 'l michelson) : bool * 'l michelson =
  match node with
  | Seq (loc, items) ->
  ...
  | Prim (loc, p, [ bt; bf ], annot) when is_cond p ->
    ...
    (match last_bt, last_bf with
     ...
     | _ -> branch_changed, Prim (loc, p, [ bt; bf ], annot))
