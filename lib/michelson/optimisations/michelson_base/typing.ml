...
let unify_annots pref ?tolerant a b =
  ...
  | Some a, Some b ->
    if tolerant = Some ()
    then Ok None
    else Error (sprintf "Cannot unify annotations '%s%s' and '%s%s'" pref a pref b)
  | _ -> Ok None
;;
...
let rec unify_types ?tolerant t u =
  ...
  let open Result in
  let err () =
    Error (sprintf "Cannot unify types '%s' and '%s'." (show_mtype t) (show_mtype u))
  in
...
let is_packable = cata_mtype (fun ?annot_type:_ ?annot_variable:_ -> is_packable_f)
