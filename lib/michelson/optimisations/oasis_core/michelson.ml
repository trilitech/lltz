...
let cata_mtype_stripped f = cata_mtype (fun ?annot_type:_ ?annot_variable:_ x -> f x)
...
let sequence_instr_f =
  ...
  | ( MI0 _ | MI1 _ | MI1_fail _ | MI2 _
    | MI3
        (Slice | Update | Get_and_update | Transfer_tokens | Check_signature | Open_chest)
    | MIdrop
    | MIswap
    | MIerror _
    | MIcomment _
...
let cata { f_instr; f_literal } =
  let rec cata_instr { instr } = f_instr (map_instr_f cata_instr cata_literal instr)
...
module MLiteral = struct
  ...
  let mk_map xs =
    { literal =
        AnyMap
          (Base.List.dedup_and_sort ~compare:(fun (k1, _) (k2, _) -> compare k1 k2) xs)
    }
  ;;
  ...
  let rec to_michelson_string instr_to_string ~protect : literal -> string =
    ...
    fun { literal } ->
      ...
      | Pair (l, r) ->
        prot
          ~protect
          (sprintf "Pair %s %s" (continue ~protect:true l) (continue ~protect:true r))
      | None_ -> "None"
      ...
      | AnyMap xs ->
        ...
        continue ~protect xs
      | Elt (k, v) ->
        sprintf "Elt %s %s" (continue ~protect:true k) (continue ~protect:true v)
      | Instr i -> instr_to_string i
      | Lambda_rec i -> prot ~protect (sprintf "Lambda_rec %s" (instr_to_string i))
...
let string_of_mtype ?full ?human ?protect ?annot ~html t =
  Misc.with_buffer (fun b -> buffer_mtype ?full ?human ?protect ?annot ~html b t)
;;
...
let memo_string_of_mtype_human =
  Misc.memoize ~clear_after:1000 (fun _f (full, se) ->
    string_of_mtype ?full:(if full then Some () else None) ~human:() ~html:false se)
;;

let memo_string_of_mtype_human ?full t = memo_string_of_mtype_human (full = Some (), t)
...
type ('i, 'l) talg =
  { f_tinstr :
      stack_in:stack Result.t -> stack_out:stack Result.t -> ('i, 'l) instr_f -> 'i
...
let mk_spec name ?commutative ?arities rule =
  let rule ~tparameter stack instr =
    let err msg =
      let tinstr =
        map_instr_f (fun x -> x (Error (name ^ " error"))) (fun _ -> assert false) instr
      in
      tinstr, Error msg
    in
...
let mk_spec_no_sub name ?commutative ?arities rule =
  let rule ~tparameter stack instr =
    let tinstr = map_instr_f (fun _ -> assert false) (fun _ -> assert false) instr in
...
let mk_spec_const name t = mk_spec_basic ~arities:(0, 1) name (fun _ -> Some [ t ])
...
let unify_stacks s1 s2 =
  match s1, s2 with
  | Stack_ok s1, Stack_ok s2 -> Option.map (fun x -> Stack_ok x) (unify_ok_stacks s1 s2)
...
let mi_join_tickets =
  mk_spec_basic "JOIN_TICKETS" ~arities:(1, 1) (function
    | { mt =
          MT2 (Pair _, ({ mt = MT1 (Ticket, _) } as t1), ({ mt = MT1 (Ticket, _) } as t2))
      }
      :: _
...
let mi_pairing_check =
  mk_spec_basic "PAIRING_CHECK" ~arities:(1, 1) (function
    | { mt =
          MT1
            ( List
            , { mt = MT2 (Pair _, { mt = MT0 Bls12_381_g1 }, { mt = MT0 Bls12_381_g2 }) }
            )
      }
      :: _ -> Some [ mt_bool ]
    | _ -> None)
...
let mi_dipn =
  let rule ~tparameter:_ stack = function
    | MIdipn (n, body) when n <= List.length stack ->
      ...
      (match body.stack_out with
       | Ok (Stack_ok stack') -> Some (tinstr, Ok (Stack_ok (List.take n stack @ stack')))
       | _ -> Some (tinstr, Error "DIP: body error"))
    | _ -> assert false
  in
  mk_spec "DIPn" rule
...
let mi_dup i =
  ...
  let rec get acc n = function
    | x :: _ when n = 1 -> if is_duppable x then Some (x :: List.rev (x :: acc)) else None
    ...
    | c :: tail, MIiter body ->
      ...
      (match el with
       ...
       | Some el ->
         ...
         let ok =
           match body.stack_out with
           | Ok (Stack_ok stack') when unifiable_ok_stacks tail stack' -> true
           | Ok Stack_failed -> true
           | _ -> false
         in
         let s = if ok then Ok (Stack_ok tail) else Error "ITER: incompatible body" in
         Some (tinstr, s))
    | [], MIiter _ -> None
    | _ -> assert false
  in
    ...
    | MIloop body ->
      (match stack with
       | { mt = MT0 Bool } :: tail ->
         ...
         (match body.stack_out with
          | Ok (Stack_ok ({ mt = MT0 Bool } :: tail')) when unifiable_ok_stacks tail tail'
...
let mi_get =
  mk_spec_basic "GET" ~arities:(2, 1) (function
    ...
    | key :: { mt = MT2 (Big_map, key', value) } :: _ when unifiable_types key key' ->
      Some [ mt_option value ]
    | _ -> None)
;;
...
let rec is_comparable mtype =
  ...
  | MT2 (Pair _, t1, t2) | MT2 (Or _, t1, t2) -> is_comparable t1 && is_comparable t2
  | MT0
      ( Operation
      | Sapling_state _
...
let mi_update =
  mk_spec_basic "UPDATE" ~arities:(3, 1) (function
    | k :: { mt = MT0 Bool } :: { mt = MT1 (Set, k') } :: _ ->
      Option.map (fun k -> [ mt_set k ]) (Result.get_ok (unify_types ~tolerant:() k k'))
...
let mi_mem =
  mk_spec_basic "MEM" ~arities:(2, 1) (function
    ...
    | k :: { mt = MT2 (Map, k', _) } :: _ when unifiable_types k k' -> Some [ mt_bool ]
...
let mi_sapling_verify_update =
  mk_spec_basic "SAPLING_VERIFY_UPDATE" ~arities:(2, 1) (function
    | { mt = MT0 (Sapling_transaction { memo = m1 }) }
      ...
      :: _
      when m1 = m2 ->
      Some [ mt_option (mt_pair mt_bytes (mt_pair mt_int (mt_sapling_state m1))) ]
    | _ -> None)
;;
...
let mi_size =
  mk_spec_basic "SIZE" ~arities:(1, 1) (function
    | { mt = MT0 String | MT0 Bytes | MT1 ((Set | List), _) | MT2 ((Map | Big_map), _, _)
      }
      :: _ -> Some [ mt_nat ]
    | _ -> None)
;;
...
let mi_self =
  let rule ~tparameter stack = function
    | MI0 (Self ep_name) ->
      ...
      let mt_contract t = { (mt_contract t) with annot_variable = Some "self" } in
      (match ep_name with
       | None -> Some (tinstr, Ok (Stack_ok (mt_contract (fst tparameter) :: stack)))
       | Some ep_name ->
    ...
    | MIcreate_contract { tparameter; tstorage; code; views } ->
      (match stack with
       | { mt = MT1 (Option, { mt = MT0 Key_hash }) }
         ...
         :: storage
         :: tail ->
         let code = code (Ok (initial_stack ~tparameter:(fst tparameter) ~tstorage)) in
         let views = List.map (typecheck_view ~tstorage) views in
         if unifiable_types storage tstorage
         then (
           let tinstr = MIcreate_contract { tparameter; tstorage; code; views } in
...
let spec_of_prim2 ~protocol p =
  let mk ?commutative name =
    ...
    | instances ->
      let f = function
        | x1 :: x2 :: _ ->
          (match List.assoc_opt (strip_annots x1, strip_annots x2) instances with
           | None -> None
           | Some t -> Some [ t ])
        | _ -> None
      in
...
let typecheck_literal_f ~tparameter:_ (literal : _ literal_f) (t : mtype Result.t) =
  let tliteral' : (stack Result.t -> tinstr, mtype Result.t -> tliteral) literal_f =
  ...
  let r =
    ...
    | MT2 (Pair _, fst, snd), Pair (x1, x2) -> return (Pair (x1 (Ok fst), x2 (Ok snd)))
    ...
    | MT1 ((Set | List), t), Seq xs -> return (Seq (List.map (fun x -> x (Ok t)) xs))
    ...
    | MT2 (Lambda, t_in, _), Instr i -> return (Instr (i (Ok (Stack_ok [ t_in ]))))
    | MT2 (Lambda, _t1, _), Seq (_ :: _) -> assert false (* eliminated by 'sanitize' *)
  ...
  | Error msg ->
    ...
    let tliteral = map_literal_f (fun f -> f err) (fun f -> f err) tliteral' in
    { tliteral; t = Error msg }
;;

let typecheck_instr_f ~protocol ~tparameter i (stack_in : stack Result.t) : tinstr =
...
let has_error ~path ~accept_missings =
  let has_missing_type t = if accept_missings then [] else has_missing_type ~path t in
    ...
    | Ok _ ->
      (match instr with
       ...
       | MIlambda (t1, t2, _i) as x ->
         has_missing_type t1 @ has_missing_type t2 @ fold_instr_f ( @ ) (curry fst) [] x
...
let display_view display_instr { name; tparameter; treturn; onchain_code } =
  ...
  | Some code ->
    sprintf
      "\nview\n  %S %s %s\n%s;"
      name
      (string_of_mtype ~protect:() ~html:false (Option.default mt_unit tparameter))
...
let buffer_f ~protocol ~html ~show_stack ~new_line b =
  ...
  let f_tliteral ~t:_ x indent protect =
    ...
    | Elt (k, v) -> elt (k, v)
    | AnyMap xs ->
      Misc.buffer_protect b true "{" "}" (fun () -> Misc.buffer_concat b "; " elt xs)
  ...
  let f_tinstr ~stack_in:_ ~stack_out x ~sub indent =
    ...
    (match x with
     ...
     | MI2 (View (name, t)) ->
       with_stack (fun () -> out "VIEW %S %s" name (string_of_mtype ~protect:() ~html t))
     | MIerror error ->
       do_indent ();
     ...
     | MI0 (Empty_bigmap (k, v) | Empty_map (k, v)) ->
       with_stack (fun () ->
         ...
         out_str
           (String.concat
              " "
              [ string_of_mtype ~protect:() ~html k; string_of_mtype ~protect:() ~html v ]))
     ...
     | MIpush (t, l) ->
       with_stack (fun () ->
         ...
         l indent true)
     | MI0 (Self (Some entrypoint)) -> with_stack (fun () -> out "SELF %%%s" entrypoint)
     | MIlambda (t1, t2, l) ->
       line "LAMBDA";
...
let _buffer_tliteral ~protocol ~html ~show_stack b indent protect x =
  cata_tliteral (buffer_f ~protocol ~html ~show_stack ~new_line:true b) x indent protect
;;
...
let seq_snoc xs x =
  ...
  | xs -> { tinstr = MIseq [ xs; x ]; stack_in = xs.stack_in; stack_out = x.stack_out }
;;
...
let display_tinstr ~protocol ~show_stack ~new_line indent inst =
  ...
  Misc.with_buffer (fun b ->
    buffer_tinstr ~protocol ~html:false ~show_stack ~sub:true ~new_line b indent inst)
;;

let render_tinstr ~protocol ~show_stack indent inst =
  let inst = insert_subsequences inst in
  Misc.with_buffer (fun b ->
    buffer_tinstr ~protocol ~html:true ~show_stack ~sub:true ~new_line:true b indent inst)
;;
...
let pretty_instr_f ~protocol i wrap ppf =
  ...
  | MI2 (Updaten n)
  | MIdup n
  | MI0 (Sapling_empty_state { memo = n }) -> wrap (fun ppf -> fprintf ppf "%t %d" name n)
  ...
  | MI1 (Contract (None, t) | Unpack t | Left (_, _, t) | Right (_, _, t)) ->
    wrap (fun ppf -> fprintf ppf "%t %s" name (string_of_mtype ~protect:() ~html:false t))
  | MI1 (Rename a) ->
  ...
  | MI1 (Contract (Some ep, t)) ->
    wrap (fun ppf ->
      fprintf ppf "CONTRACT %%%s %s" ep (string_of_mtype ~protect:() ~html:false t))
  | MI2 (View (name, t)) ->
    wrap (fun ppf ->
      fprintf ppf "VIEW %S %s" name (string_of_mtype ~protect:() ~html:false t))
  | MIlambda (t1, t2, c) ->
    wrap (fun ppf ->
  ...
  | MIcreate_contract { tparameter = tparameter, annot; tstorage; code; views } ->
    ...
    wrap (fun ppf ->
      fprintf
        ppf
        "CREATE_CONTRACT { parameter %s; storage %s; code %t%s}"
  ...
  | MIconcat1
  | MIconcat2
  | MIconcat_unresolved -> wrap (fun ppf -> fprintf ppf "%t" name)
;;
...
let string_of_literal ~protocol m = Format.asprintf "%t" (pretty_literal ~protocol m true)
...
module Of_micheline = struct
      ...
      (match name, arguments with
       ...
       | "Pair", l ->
         (match List.rev l with
          | [] -> assert false
          | a :: l ->
            List.fold_left (fun x y -> MLiteral.pair (literal y) x) (literal a) l)
       | "None", [] -> MLiteral.none
  ...
  and instruction x =
    let err () = MIerror (sprintf "Cannot parse instruction %S" (Micheline.show x)) in
    let cmp instr = MIseq [ { instr = MI2 Compare }; { instr } ] in
    let fail =
      MIseq [ { instr = MIpush (mt_unit, MLiteral.unit) }; { instr = MI1_fail Failwith } ]
    in
    let if_op instr x y =
    ...
    let parse_simple_macro = function
      ...
      | prim
        when String.index_opt prim 'D' = Some 0
             ...
             && Base.String.count prim ~f:(fun c -> c = 'U') = String.length prim - 2 ->
      ...
      | Primitive { name; annotations; arguments } ->
        (match name, arguments with
         | "RENAME", [] ->
           ...
           MI1 (Rename a)
         | "UNIT", [] -> MI0 Unit_
         | "EMPTY_MAP", [ k; v ] -> MIpush (mt_map (mtype k) (mtype v), MLiteral.mk_map [])
         ...
         | prim, [ x ]
           when String.index_opt prim 'D' = Some 0
                && String.index_opt prim 'P' = Some (String.length prim - 1)
                && 2 < String.length prim
                && Base.String.count prim ~f:(fun c -> c = 'I') = String.length prim - 2
         ...
         | "CONTRACT", [ t ] ->
           let entrypoint =
             match annotations with
             | [] -> None
             | entrypoint :: _ -> Base.String.chop_prefix ~prefix:"%" entrypoint
           in
         ...
         | "LAMBDA_REC", [ t; u; x ] -> MIlambda_rec (mtype t, mtype u, instruction x)
         | "EMIT", _ ->
         ...
         | "CREATE_CONTRACT", [ x ] ->
           let tparameter, tstorage, code =
             ...
             | Sequence
               ...
               ( (mtype tparameter, None (* TODO single entrypoint annotation *))
               , mtype tstorage
               , instruction code )
             | _ -> assert false
           in
           MIcreate_contract { tparameter; tstorage; code; views = (* TODO *) [] }
         | "SELF", [] ->
           let entrypoint =
             match annotations with
             | [] -> None
  ...
  let contract =
    let read_element (p, s, c, vs) = function
      ...
      | Primitive { name } -> failwith ("ill-formed contract: unexpected '" ^ name ^ "'")
    ...
    | Micheline.Sequence s ->
      (match List.fold_left read_element (None, None, None, []) s with
       ...
       | Some tparameter, Some tstorage, Some code, views ->
         ...
         ({ contract = { tparameter; tstorage; code; views }; storage = None } : instance))
...
module To_micheline = struct
  ...
  let mtype ?annot_field =
    cata_mtype ~annot_field (fun ?annot_type ?annot_variable t ~annot_field ->
      let annotations =
        ...
        List.somes [ get "%" annot_field; get ":" annot_type; get "@" annot_variable ]
      in
  ...
  let rec c_ad_r s =
    ...
    | exception _ -> []
    | other -> Format.kasprintf failwith "c_ad_r macro: wrong char: '%c' (of %S)" other s
  ;;
  ...
  let rec set_c_ad_r s =
    ...
    | 'A' when String.length s > 1 ->
      [ primitive "DUP" []
      ; dip_seq (primitive "CAR" [] :: set_c_ad_r (Base.String.drop_prefix s 1))
      ; primitive "CDR" []
      ; primitive "SWAP" []
      ; primitive "PAIR" []
      ]
    ...
    | 'D' when String.length s > 1 ->
      [ primitive "DUP" []
      ; dip_seq (primitive "CDR" [] :: set_c_ad_r (Base.String.drop_prefix s 1))
      ; primitive "CAR" []
      ; primitive "PAIR" []
      ]
    | 'D' -> [ primitive "CAR" []; primitive "PAIR" [] ]
    | exception _ -> Format.kasprintf failwith "set_c_r_macro: called with no chars: S" s
    | other -> Format.kasprintf failwith "set_c_r_macro: wrong char: '%c' (of %S)" other s
  ;;
  ...
  and instruction ~protocol (the_instruction : instr) =
    ...
    | MIdipn (n, instr) -> primn "DIP" [ int (string_of_int n); rec_instruction instr ]
    ...
    | MI0 (Self (Some entrypoint)) -> primn ~annotations:[ "%" ^ entrypoint ] "SELF" []
    | MI2 (Pair (a1, a2)) -> primn ~annotations:(two_field_annots (a1, a2)) "PAIR" []
    | MI1 (Right (a1, a2, mty)) ->
    ...
    | MIlambda (t1, t2, b) -> primn "LAMBDA" [ mtype t1; mtype t2; rec_instruction b ]
    ...
    | MIcreate_contract { tparameter = tparameter, annot_field; tstorage; code; views } ->
      primn
        "CREATE_CONTRACT"
        [ sequence
    ...
    | ( MI0
      ...
      | MI3
          ...
          | Check_signature
          | Open_chest )
      | MIdrop | MIswap | MImich _ | MIconcat1 | MIconcat2 | MIconcat_unresolved ) as
      simple ->
      (try prim0 (name_of_instr_exn ~protocol simple) with
...
let string_of_tliteral ~protocol x = string_of_literal ~protocol (erase_types_literal x)

let typecheck_contract ~protocol ({ tparameter; tstorage; code; views } : contract) =
  let code =
    typecheck_instr
      ~protocol
      ~tparameter
  ...
  let code =
    ...
    | Ok (Stack_ok [ { mt = MT2 (Pair _, fst, snd) } ])
      when unifiable_types fst (mt_list mt_operation) && unifiable_types snd tstorage ->
      code
    | Ok Stack_failed -> code
    | Ok _ ->
      let msg = unexpected_final_stack_error in
      let err =
        { tinstr = MIerror msg; stack_in = code.stack_in; stack_out = code.stack_out }
      in
      seq_snoc code err
    | Error _ -> code
  in
...
let typecheck_instance ~protocol ({ contract; storage } : instance) : tinstance =
  let ({ tparameter; tstorage } as contract) = typecheck_contract ~protocol contract in
  let storage = Option.map (typecheck_literal ~protocol ~tparameter tstorage) storage in
  { contract; storage }
;;

let has_error_tinstance
  ~accept_missings
  ...
  let errors =
    ...
    in
    let e =
      has_error ~path:"code" ~accept_missings code @ List.concat_map has_error_view views
    in
    e
       ...
       (fun acc { name; tparameter; treturn; onchain_code } ->
         ...
         | Some code ->
           sprintf
             ...
             acc
             name
             (string_of_mtype ~protect:() ~html:true (Option.default mt_unit tparameter))
...
let profile ~protocol =
  ...
  let same x y =
    match x, y with
    | Some x, Some y -> if x = y then return (Some x) else error "profile: unequal d"
  ...
  let f_instr i =
    ...
    | MIseq xs ->
      let f = function
        ...
        | (p1, Some d1), Ok (p2, Some d2) -> return (max p1 (p2 - d1), Some (d1 + d2))
...
let has_arity ~protocol a instr = has_profile ~protocol (profile_of_arity a) instr
let arity ~protocol instr = Result.map arity_of_profile (profile ~protocol instr)
...
let rec mtype_examples t =
  ...
  | MT0 Key_hash ->
    [ MLiteral.string "tz1..."; MLiteral.string "tz2..."; MLiteral.string "tz3..." ]
  ...
  | MT1 (Option, t) -> List.map MLiteral.some (mtype_examples t) @ [ MLiteral.none ]
  ...
  | MT2 (Pair _, fst, snd) ->
    ...
    (match l1, l2 with
     ...
     | _ ->
       List.fold_left
         (fun acc b -> List.fold_left (fun acc a -> MLiteral.pair a b :: acc) acc l1)
         []
         l2)
  | MT2 (Or _, left, right) ->
    ...
    (match l1, l2 with
     | a1 :: a2 :: _, b1 :: b2 :: _ ->
       [ MLiteral.left a1; MLiteral.left a2; MLiteral.right b1; MLiteral.right b2 ]
     | _ ->
       List.fold_left
         (fun acc b -> List.fold_left (fun acc a -> MLiteral.pair a b :: acc) acc l1)
         []
...
let display_instr ~protocol t1 instr =
  let tinstr = typecheck_instr ~tparameter:(mt_unit, None) (Stack_ok [ t1 ]) instr in
  display_tinstr ~protocol ~show_stack:false ~new_line:false 0 (tinstr ~protocol)
;;
