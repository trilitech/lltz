...
let mi_dipn =
  let rule ~tparameter:_ stack = function
    | MIdipn (n, body) when n <= List.length stack ->
      ...
      (match body.stack_out with
       | Ok (Stack_ok stack') -> Some (tinstr, Ok (Stack_ok (take n stack @ stack')))
       | _ -> Some (tinstr, Error "DIP: body error"))
    | _ -> assert false
  in
  mk_spec "DIPn" rule
...
let mi_min_block_time = mk_spec_const "MIN_BLOCK_TIME" mt_nat
...
let mi_mich ~name ~types_in ~types_out =
  mk_spec_basic
    ...
    (fun stack -> if is_prefix equal_mtype types_in stack then Some types_out else None)
;;
...
let spec_of_prim2 p =
  let mk ?commutative name =
    ...
    | instances ->
      let f = function
        | x1 :: x2 :: _ ->
          (match assoc_opt (strip_annots x1, strip_annots x2) instances with
           | None -> None
           | Some t -> Some [ t ])
        | _ -> None
      in
      ...
      (match name, arguments with
       ...
       | "Pair", l ->
         (match List.rev l with
          | [] -> assert false
          | a :: l ->
            List.fold_left ~f:(fun x y -> MLiteral.pair (literal y) x) ~init:(literal a) l)
       | "None", [] -> MLiteral.none
    ...
    let instr =
      ...
      | Primitive { name; annotations; arguments } ->
        (match name, arguments with
         ...
         | "UNPAIR", [ Int n ] -> MIunpair (List.init (int_of_string n) (fun _ -> true))
         | "CAR", [] -> MIfield [ A ]
         | "CDR", [] -> MIfield [ D ]
         | "CONTRACT", [ t ] ->
           let entrypoint =
         ...
         | ( "SAPLING_EMPTY_STATE"
           , [ Primitive { name = "int"; annotations = annots; arguments = args } ] ) ->
           err ()
  ...
  let mtype ?annot_field =
    cata_mtype ~annot_field (fun ?annot_type ?annot_variable t ~annot_field ->
      let annotations =
        ...
        somes [ get "%" annot_field; get ":" annot_type; get "@" annot_variable ]
      in
