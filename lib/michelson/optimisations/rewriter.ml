...
let rec micheline_to_oasis_micheline (node : (unit, Michelson.Ast.Prim.t) Tezos_micheline.Micheline.node) : Oasis_core.Micheline.t =
  ...
  | Seq (_, nodes) ->
      Sequence (List.map ~f:(micheline_to_oasis_micheline) nodes)
...
let rec strip_annotations (x : (unit, Michelson.Ast.Prim.t) Tezos_micheline.Micheline.node) : (unit, Michelson.Ast.Prim.t) Tezos_micheline.Micheline.node =
...
let optimise_micheline ?(strip_annots=true) (node : (unit, Michelson.Ast.Prim.t) Tezos_micheline.Micheline.node) : (unit, Michelson.Ast.Prim.t) Tezos_micheline.Micheline.node =
  ...
  if strip_annots then
    strip_annotations micheline
  else
    micheline
