...
let lltz_specific (expr : instr_list) : (instr_list * instr_list) option =
  ...
  | MIpush ({ mt = MT1 (List, t) }, { literal = Seq xs }) :: rest when List.length xs = 1
    ->
    [ MI0 (Nil t) ]
    @ List.concat (List.map ~f:(fun x -> [ push_instr t x; MI2 Cons ]) (List.rev xs))
    $ rest
  (* LAMBDA int int { constant "hash..."} -> PUSH (lambda int int) (constant "hash...") *)
  | MIlambda
      ( { mt = MT0 Int }
      , { mt = MT0 Int }
      , { instr = MIConstant { literal = String hash } } )
    :: rest -> [ MIpush (mt_lambda mt_int mt_int, MLiteral.constant hash) ] $ rest
